...
assert(parity.includes("version:'2.1.0-zoom-host-controls'"),'Zoom-style adaptive dock engine version is missing.');
...
assert(zoomBehavior.includes("version:'1.2.0'"),'Zoom behavior guard version is missing.');
...
assert(zoomBehavior.includes("add('Mute'")&&zoomBehavior.includes("add('Ask to Unmute'")&&zoomBehavior.includes("add('Stop Video'"),'Host/co-host participant More menu must expose real audio/video controls.');
assert(zoomBehavior.includes("meeting.renameParticipant(participantId,name)")&&zoomBehavior.includes("add('Rename'"),'Participant rename must use backend authority from the More menu.');
assert(zoomBehavior.includes("if(isHost)add(targetRole==='cohost'?'Remove Co-host':'Make Co-host'"),'Only the host may add or remove co-host authority.');
assert(zoomBehavior.includes("senderAuthority")&&zoomBehavior.includes("['host','cohost'].includes(senderRole)"),'Remote host-control signals must verify sender role before touching local media.');
assert(zoomBehavior.includes("media()?.setMicrophone?.(false)")&&zoomBehavior.includes("media()?.setCamera?.(false)"),'Mute and Stop Video commands must operate through the clean local media authority.');
assert(zoomBehavior.includes("zoomUnmuteRequestDialog")&&zoomBehavior.includes("asked you to unmute"),'Ask to Unmute must require participant consent rather than forcing unmute.');
assert(zoomBehavior.includes("setSpotlight(participantId)")&&parity.includes("spotlightParticipantId")&&parity.includes("spotlightTile||q('#remoteTileStrip .remote-peer-tile.active-speaker')"),'Spotlight must override active-speaker promotion through the meeting stage authority.');
