...
assert(macOverlay.includes('function parkCaptureOwnerBehindVideoDock')&&macOverlay.includes('videoWindow.getBounds()')&&macOverlay.includes('main.setAlwaysOnTop(false)'),'Physical Mac presenter mode must keep the exact capture owner composited directly behind the native right-side video dock.');
