(()=>{
  ...
  function normalizeToolbar(){const footer=q('.meeting-footer');if(!footer)return false;ensureFeatureControls();const micAttached=attachCaret(q('#roomMic')),videoAttached=attachCaret(q('#roomCamera'));for(const caret of footer.querySelectorAll('.av-device-caret')){const target=caret.dataset.controlFor&&q(`#${caret.dataset.controlFor}`);if(target&&target.nextElementSibling!==caret)target.insertAdjacentElement('afterend',caret);}window.DominionMeetingParity?.install?.();return micAttached&&videoAttached;}
