...
try{
  ...
  await evaluate(`window.DominionMeetingParity.decorateControls();window.DominionApprovedReferenceParity.sync();window.DominionRuntimeStability.sync();window.DominionZoomScreenshotReference.sync();window.DominionRuntimeStability.ensureToolbarZones();true`);
  await waitFor("document.querySelector('#roomHostTools')&&!document.querySelector('#roomHostTools').hidden",'host tools control');
  await waitFor("(()=>{const footer=document.querySelector('.meeting-footer');if(!footer)return false;const h=footer.getBoundingClientRect().height;return h>=55&&h<=57;})()",'final 2.0.41 reference toolbar height',5000);
  ...
  assert.ok(toolbar.footer.height>=63&&toolbar.footer.height<=65,`Meeting toolbar must remain at the approved 2.0.45 physical reference height; received ${toolbar.footer.height}px.`);
  for(const key of ['mic','camera','share','participants','chat','reactions','more']){assert.ok(toolbar[key].icon>=22.5,`${key} icon is below the approved 2.0.41 reference size.`);assert.ok(toolbar[key].label>=10,`${key} label is below the approved 2.0.41 reference size.`);}
  ...
  assert.ok(toolbar.host.label>=10,'Host Tools must retain the approved compact Zoom-reference label size.');
  const reactIcon=await evaluate(`(()=>{window.DominionZoomPhysicalAcceptance?.sync?.();const icon=document.querySelector('#roomReactions .ds-control-icon');return {marker:icon?.dataset.dsReactionIcon||'',svg:icon?.querySelectorAll('svg').length||0,paths:icon?.querySelectorAll('path').length||0,circles:icon?.querySelectorAll('circle').length||0};})()`);
  assert.equal(reactIcon.marker,'executive-reaction-face','React must use the canonical reaction-face toolbar icon.');
  assert.equal(reactIcon.svg,1,'React must render one toolbar glyph only.');
  assert.ok(reactIcon.paths>=1&&reactIcon.circles>=3,'React face glyph must contain one face, two eyes, and a smile.');

  await evaluate(`document.querySelector('#roomParticipants').click();true`);await sleep(190);
  const participants=await evaluate(`(()=>{window.DominionZoomScreenshotReference.sync();window.DominionRuntimeStability.layoutSideSurface();const panel=document.querySelector('.room-side'),pr=panel.getBoundingClientRect(),br=document.querySelector('.meeting-body').getBoundingClientRect(),sr=document.querySelector('.stage').getBoundingClientRect(),searchWrap=panel.querySelector('.zoom-participant-search'),search=searchWrap?.querySelector('input'),head=panel.querySelector('.room-side-head strong'),footer=panel.querySelector('.ds-ref-participants-footer'),count=panel.querySelectorAll('#participantRoster [data-participant-id]').length,legacy=panel.querySelectorAll('.zoom-participant-footer,#participantBulkActions');return {count,width:Math.round(pr.width),height:Math.round(pr.height),position:getComputedStyle(panel).position,mode:panel.dataset.dsRuntimeMode||'',inside:pr.left>=br.left+10&&pr.right<=br.right-10&&pr.top>=br.top+10&&pr.bottom<=br.bottom-10,centerDelta:Math.round(Math.abs((pr.left+pr.width/2)-(br.left+br.width/2))),searchVisible:Boolean(searchWrap&&!searchWrap.hidden&&getComputedStyle(searchWrap).display!=='none'),searchFont:search?parseFloat(getComputedStyle(search).fontSize):0,searchHeight:search?search.getBoundingClientRect().height:0,headFont:head?parseFloat(getComputedStyle(head).fontSize):0,footerVisible:Boolean(footer&&!footer.hidden),footerCount:panel.querySelectorAll('.ds-ref-participants-footer').length,legacyCount:legacy.length,stageRightGap:Math.round(br.right-sr.right)};})()`);
  ...
  assert.ok(participants.centerDelta<=8,'Participants must open at one stable centered floating position.');
  ...
  assert.equal(participants.footerVisible,true,'Final Participants Invite / Mute all / More footer is missing.');
  assert.equal(participants.footerCount,1,'Participants must render exactly one final action footer.');
  assert.equal(participants.legacyCount,0,'Legacy participant action footers must not coexist with the final footer.');
  ...
  console.log('DOMINIONSTAR_PACKAGED_ZOOM_VISUAL_OK stable-three-zone-toolbar reference-footer-64 icons-23 labels-10.5 audio-left video-left actions-centered end-right share-green host-tools participants-floating compact-reference-text reactions-left more-reference-text no-uncaught-renderer-errors');
