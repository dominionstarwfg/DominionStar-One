...
assert.ok(runtime.includes('function openRuntimeHostTools(anchor)')&&runtime.includes('function installRuntimeHostToolsAuthority()')&&runtime.includes('void openRuntimeHostTools(host)')&&!runtime.includes("DominionMeetingParity?.openSecurity?.(hostTools)"),'Host Tools must open through one direct runtime-owned toolbar authority.');
...
assert.ok(runtime.includes("for(const name of ['DominionZoomAdaptiveParity','DominionZoomParticipantsReference2041','DominionZoomProductionPolish','DominionApprovedReferenceParity','DominionZoomBehavior','DominionZoomPhysicalAcceptance'])"),'All known periodic layout authorities, including the legacy participant reconciler, must be retired by the final runtime.');
