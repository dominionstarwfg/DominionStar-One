...
for(const channel of ['meeting:create','meeting:personal-room','meeting:update-personal-room','meeting:start-personal-room','meeting:schedule','meeting:list-schedules','meeting:start-schedule','meeting:cancel-schedule','meeting:request-join','meeting:join-status','meeting:mark-joined','meeting:leave','meeting:host-queue','meeting:decide','meeting:snapshot','meeting:set-cohost','meeting:remove-participant','meeting:rename-participant','meeting:set-security','meeting:set-chat-policy','meeting:set-caption-state','meeting:publish-caption','meeting:get-transcript','meeting:transfer-host-and-leave','meeting:end'])assert(main.includes(channel),`Missing IPC owner ${channel}`);
for(const method of ['create:','personalRoom:','updatePersonalRoom:','startPersonalRoom:','schedule:','listSchedules:','startSchedule:','cancelSchedule:','requestJoin:','joinStatus:','markJoined:','leave:','hostQueue:','decide:','snapshot:','setCohost:','removeParticipant:','renameParticipant:','setSecurity:','setChatPolicy:','setCaptionState:','publishCaption:','transcript:','transferHostAndLeave:','end:'])assert(preload.includes(method),`Missing renderer bridge method ${method}`);
...
console.log('DOMINIONSTAR_MEET_V2_LIFECYCLE_OK personal-id generated-id passcode-3-7 reusable-waiting recurring-identity schedule prejoin waiting admit cohost atomic-host-handoff raise-hand-authority captions transcript-retention leave end');
