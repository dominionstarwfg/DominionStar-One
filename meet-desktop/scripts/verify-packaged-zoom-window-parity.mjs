...
async function target(){const deadline=Date.now()+15000;while(Date.now()<deadline){if(child.exitCode!==null)throw new Error(`Packaged app exited before adaptive DominionStar gate.\n${stderr}`);try{const response=await fetch(`http://127.0.0.1:${port}/json/list`,{signal:AbortSignal.timeout(700)});if(response.ok){const targets=await response.json();const page=targets.find(item=>item.type==='page'&&String(item.url||'').startsWith('file://'));if(page?.webSocketDebuggerUrl)return page;}}catch{}await sleep(150);}throw new Error('Unable to attach to packaged renderer for adaptive DominionStar gate.');}
function connect(url){return new Promise((resolve,reject)=>{const socket=new WebSocket(url);const timer=setTimeout(()=>reject(new Error('Adaptive DominionStar gate CDP connection timeout.')),3000);socket.addEventListener('open',()=>{clearTimeout(timer);resolve(socket);},{once:true});socket.addEventListener('error',()=>{clearTimeout(timer);reject(new Error('Adaptive DominionStar gate CDP connection failed.'));},{once:true});});}
...
try{
  ...
  const desktopParticipants=await evaluate(`(()=>{window.DominionRuntimeStability.syncParticipantsSurface();window.DominionRuntimeStability.layoutSideSurface();const panel=document.querySelector('.room-side'),body=document.querySelector('.meeting-body'),stage=document.querySelector('.stage'),pr=panel.getBoundingClientRect(),br=body.getBoundingClientRect(),sr=stage.getBoundingClientRect(),search=panel.querySelector('.zoom-participant-search'),waiting=document.querySelector('#waitingQueueSection');return {innerWidth,mode:panel.dataset.dsRuntimeMode,width:Math.round(pr.width),rightGap:Math.round(br.right-pr.right),inside:pr.left>=br.left+10&&pr.right<=br.right-10&&pr.top>=br.top+10&&pr.bottom<=br.bottom-10,stageRightGap:Math.round(br.right-sr.right),searchHidden:Boolean(search?.hidden||getComputedStyle(search).display==='none'),waitingHidden:Boolean(waiting?.hidden||getComputedStyle(waiting).display==='none'),heading:panel.querySelector('.room-side-head strong')?.textContent||''};})()`);
  ...
  assert.doesNotMatch(stderr,/Uncaught\s+(?:NotFoundError|TypeError|ReferenceError|SyntaxError)/i,'Adaptive DominionStar gate detected an uncaught renderer error.');
  console.log('DOMINIONSTAR_PACKAGED_ADAPTIVE_WINDOW_PARITY_OK compact-prejoin right-default-floating-participants-chat full-stage participant-priority search-threshold adaptive-chat speaker-authority two-person-filmstrip video-3plus-visible share-video-visible no-grip');
