...
assert(parity.includes("window.addEventListener('dominion:meeting-ui-ready',()=>{install();scheduleParityRefresh();})"),'Packaged Zoom parity must install after the live meeting DOM exists and schedule event-driven refresh.');
assert(parity.includes("if(share){syncShareLayout();return;}"),'Packaged active sharing must update layout before participant media dock rebinding.');
assert(!parity.includes("setInterval(()=>{syncGreeting();"),'Packaged meeting parity must not contain the retired 700 ms reconciliation poll.');
