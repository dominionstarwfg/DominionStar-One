...
try{
  ...
  const participants=await evaluate(`(()=>{window.DominionRuntimeStability.layoutSideSurface();const panel=document.querySelector('.room-side'),pr=panel.getBoundingClientRect(),br=document.querySelector('.meeting-body').getBoundingClientRect(),sr=document.querySelector('.stage').getBoundingClientRect(),searchWrap=panel.querySelector('.zoom-participant-search'),search=searchWrap?.querySelector('input'),head=panel.querySelector('.room-side-head strong'),footer=panel.querySelector('.zoom-participant-footer'),count=panel.querySelectorAll('#participantRoster [data-participant-id]').length;return {count,rightGap:Math.round(br.right-pr.right),width:Math.round(pr.width),height:Math.round(pr.height),position:getComputedStyle(panel).position,mode:panel.dataset.dsRuntimeMode||'',searchVisible:Boolean(searchWrap&&!searchWrap.hidden&&getComputedStyle(searchWrap).display!=='none'),searchFont:search?parseFloat(getComputedStyle(search).fontSize):0,searchHeight:search?search.getBoundingClientRect().height:0,headFont:head?parseFloat(getComputedStyle(head).fontSize):0,footerVisible:Boolean(footer&&!footer.hidden),legacyVisible:Boolean(document.querySelector('#participantBulkActions')&&!document.querySelector('#participantBulkActions').hidden),stageRightGap:Math.round(br.right-sr.right)};})()`);
  assert.equal(participants.position,'absolute','Participants must remain a desktop application side surface.');
  assert.equal(participants.mode,'docked','Normal desktop-width Participants must use the approved right-side dock.');
  assert.ok(Math.abs(participants.rightGap)<=2,'Participants must dock flush to the meeting body right edge.');
  assert.ok(participants.width>=320&&participants.width<=400,'Desktop Participants width must stay readable and bounded.');
  assert.ok(participants.stageRightGap>=participants.width-2,'Opening Participants must resize the usable stage instead of covering it.');
  ...
  const chat=await evaluate(`(()=>{window.DominionRuntimeStability.layoutSideSurface();const panel=document.querySelector('#meetingChatPanel'),r=panel.getBoundingClientRect(),body=document.querySelector('.meeting-body').getBoundingClientRect(),head=panel.querySelector('header strong'),input=document.querySelector('#meetingChatInput'),policy=document.querySelector('#meetingChatPolicy'),more=panel.querySelector('.zoom-chat-more');return {rightGap:Math.round(body.right-r.right),inside:r.left>=body.left-2&&r.right<=body.right+2&&r.top>=body.top-2&&r.bottom<=body.bottom+2,width:Math.round(r.width),mode:panel.dataset.dsRuntimeMode||'',headFont:head?parseFloat(getComputedStyle(head).fontSize):0,inputFont:input?parseFloat(getComputedStyle(input).fontSize):0,policyVisible:Boolean(policy&&getComputedStyle(policy).display!=='none'),moreVisible:Boolean(more&&getComputedStyle(more).display!=='none')};})()`);
  assert.ok(chat.width>=320&&chat.width<=400,'Chat width must remain compact and readable instead of becoming an oversized form panel.');
  assert.equal(chat.inside,true,'Chat must remain inside the meeting surface.');
  assert.equal(chat.mode,'docked','Normal desktop-width Chat must use the same right-side application dock.');
  assert.ok(Math.abs(chat.rightGap)<=2,'Docked Chat must sit flush to the right edge.');
