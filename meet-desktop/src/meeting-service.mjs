...
export function createMeetingService({auth}){
  ...
  async function createRoom(input={}){
    ...
    return auth.rpc('meet_v2_create_room',{
  ...
  async function requestJoin(input={}){
    ...
    if(roomCode.length!==10)throw new Error('Meeting ID must contain 10 digits.');
    if(passcode.length<4)throw new Error('Enter the meeting passcode.');
    if(!displayName)throw new Error('Enter your display name.');
    return auth.rpc('meet_v2_request_join',{p_room_code:roomCode,p_passcode:passcode,p_display_name:displayName});
  }
  ...
  return Object.freeze({createRoom,requestJoin,joinStatus,markJoined,hostQueue,decide,snapshot,endRoom});
}
