alter table public.meet_v2_rooms
  add column if not exists annotation_enabled boolean not null default true,
  add column if not exists annotation_save_allowed boolean not null default true;

create or replace function public.meet_v2_set_annotation_policy(
  p_room_id uuid,
  p_enabled boolean,
  p_save_allowed boolean
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  set annotation_enabled=coalesce(p_enabled,true),
      annotation_save_allowed=coalesce(p_save_allowed,true),
      updated_at=now()
  where id=p_room_id
...
grant execute on function public.meet_v2_set_annotation_policy(uuid,boolean,boolean) to authenticated;
