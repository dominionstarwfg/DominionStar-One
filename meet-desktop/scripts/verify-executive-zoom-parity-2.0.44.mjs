...
assert(runtime.includes("document.addEventListener('mousemove',moveFloatingSurfaceMouse,true)")&&runtime.includes("handle.addEventListener('mousedown',event=>beginFloatingDrag(panel,event,'mouse'),true)"),'Floating panels must also support native desktop mouse/trackpad drag events.');
