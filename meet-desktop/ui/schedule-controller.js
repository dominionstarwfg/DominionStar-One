(()=>{
  ...
  function formatWhen(item){
    try{return new Intl.DateTimeFormat(undefined,{dateStyle:'medium',timeStyle:'short'}).format(new Date(`${item.date}T${item.time}`));}catch{return `${item.date||''} ${item.time||''}`.trim();}
  }
  ...
  function renderHome(items){
    ...
    let host=card.querySelector('.scheduled-home-list');
    if(!host){host=document.createElement('div');host.className='scheduled-home-list';card.append(host);}
    const empty=card.querySelector('.empty-state');
    const upcoming=items.filter(item=>whenMs(item)>=Date.now()-15*60*1000).slice(0,3);
  ...
  function render(){const items=activeItems();renderHome(items);renderMeetings(items);q('#scheduledMeetingCount')&&(q('#scheduledMeetingCount').textContent=String(items.length));}
  ...
  async function schedule(event){
    ...
    try{
      // Meet V2 currently creates the secure room immediately. Waiting Room is
      // deliberately forced on for scheduled rooms so attendees cannot enter an
      // unattended room before the host opens the scheduled meeting.
      ...
      const items=read().filter(value=>value.id!==item.id);items.push(item);write(sortItems(items));
  ...
  function start(item){
    ...
    document.body.dataset.scheduledHostStart='1';document.body.dataset.scheduledHostTitle=item.title;
    join.showModal();
    // The ordinary Join controller remains the single authority for validation,
    // prejoin media, and meeting entry. Starting a scheduled room only pre-fills it.
  }
  ...
  function clearScheduledStart(){if(q('#meetingOverlay')&&!q('#meetingOverlay').hidden){document.body.dataset.scheduledHostStart='';document.body.dataset.scheduledHostTitle='';}}
  ...
  function install(){
    ...
    document.addEventListener('click',event=>{if(event.target.closest?.('[data-open="schedule"]'))defaultSchedule();});
  ...
  const observer=new MutationObserver(()=>{decorateScheduledPrejoin();clearScheduledStart();});observer.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['hidden']});
  install();
