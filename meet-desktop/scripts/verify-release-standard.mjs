...
assert.ok(workflow.indexOf('Verify packaged adaptive Zoom meeting behavior')<workflow.indexOf('Create installable DMG, archive, and checksums'),'Installer creation must remain behind adaptive Zoom physical-reference parity.');
...
console.log(`DOMINIONSTAR_RELEASE_STANDARD_OK version=${pkg.version} dynamic-version clean-source source-cert packaged-audit packaged-launch packaged-controls zoom-render-gate physical-acceptance reaction-10s-gate physical-mac-2.0.21 adaptive-zoom-gate native-system-picker tcc-provenance installer-verify upload-last`);
