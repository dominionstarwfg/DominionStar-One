...
assert(!auth.includes('dominionstarld.com'),'Desktop auth must not contain a public-site fallback.');
assert(!auth.includes('#access_token'),'Implicit token fragments must not exist in the new desktop flow.');
assert(!auth.includes('dominionstar://auth'),'Legacy deep-link auth must not return in the clean rebuild.');
assert(main.includes("ipcMain.handle('auth:start-google'"),'Native shell must own the Google sign-in command.');
assert(preload.includes("startGoogle:()=>ipcRenderer.invoke('auth:start-google')"),'Renderer must have only the minimal auth bridge.');
...
console.log('DOMINIONSTAR_DESKTOP_AUTH_OK pkce loopback encrypted-session no-public-site-fallback');
