...
async function target(){
  ...
  while(Date.now()<deadline){
    if(child.exitCode!==null)throw new Error(`Packaged app exited before approved-reference gate.\n${stderr}`);
    try{
  ...
  throw new Error('Unable to attach to packaged renderer for approved-reference gate.');
}
...
try{
  ...
  socket.addEventListener('message',event=>{
    ...
    if(message.method==='Runtime.exceptionThrown'){runtimeErrors.push(message.params?.exceptionDetails?.exception?.description||message.params?.exceptionDetails?.text||'Runtime exception');return;}
  ...
  await waitFor("document.querySelector('.view-menu')",'View menu');
  ...
  const toolbar=await evaluate(`(()=>{window.DominionZoomProductionPolish.sync();window.DominionApprovedReferenceParity.sync();const expected=window.DominionApprovedReferenceParity.hostToolbarOrder;const footer=document.querySelector('.meeting-footer');const entries=expected.map(id=>{const node=document.querySelector('#'+id);return {id,order:Number.parseInt(getComputedStyle(node).order,10)||0,visible:Boolean(node&&!node.hidden&&getComputedStyle(node).display!=='none')};});const visual=[...entries].sort((a,b)=>a.order-b.order).map(entry=>entry.id);const raise=document.querySelector('#roomRaiseHand'),host=document.querySelector('#roomHostTools'),reactLabel=document.querySelector('#roomReactions .ds-control-label');return {visual,expected,marker:footer.dataset.approvedToolbarOrder||'',hostVisible:Boolean(host&&!host.hidden&&getComputedStyle(host).display!=='none'),raiseVisible:Boolean(raise&&!raise.hidden&&getComputedStyle(raise).display!=='none'),raiseLabel:raise?.querySelector('.ds-control-label')?.textContent||'',raiseMarker:raise?.dataset.approvedDedicatedRaiseHand||'',reactText:String(reactLabel?.textContent||'').trim(),reactFont:reactLabel?parseFloat(getComputedStyle(reactLabel).fontSize):0};})()`);
  assert.deepEqual(toolbar.visual,toolbar.expected,'Host toolbar order must be Audio → Video → Participants → Chat → React → Raise hand → Share → Host tools → More → End.');
  ...
  assert.ok(toolbar.reactFont>=11.5,'Real React label must remain readable at Zoom-scale typography.');

  await evaluate(`document.querySelector('#roomRaiseHand').click()`);await sleep(100);
  const raised=await evaluate(`(()=>{window.DominionApprovedReferenceParity.sync();const react=document.querySelector('#roomReactions .ds-control-label');return {state:window.DominionMeetingFeatures.snapshot().handRaised,label:document.querySelector('#roomRaiseHand .ds-control-label')?.textContent||'',pressed:document.querySelector('#roomRaiseHand')?.getAttribute('aria-pressed'),react:String(react?.textContent||'').trim(),reactFont:react?parseFloat(getComputedStyle(react).fontSize):0};})()`);
  ...
  assert.equal(raised.react,'React','Raising a hand must not rename the React control.');
  assert.ok(raised.reactFont>=11.5,'React label must remain readable while hand is raised.');
  ...
  await waitFor("document.querySelector('.meeting-reaction-menu')",'reaction tray');await sleep(50);
  ...
  // Chat opens with a real async snapshot-driven recipient refresh. The QA
  // fixture must let that authority settle before inserting its synthetic peer;
  // otherwise the legitimate refresh can replace the fixture option and create
  // a false failure unrelated to the approved direct-message UI.
  await evaluate(`(async()=>{window.DominionMeetingFeatures.toggleChat(true);window.DominionZoomBehavior?.dispose?.();await new Promise(resolve=>setTimeout(resolve,300));await window.DominionZoomBehavior?.refreshChatRecipients?.();window.DominionZoomAdaptiveParity.syncChat();window.DominionApprovedReferenceParity.sync();return true;})()`);
  ...
  for(const [,waiter] of pending){clearTimeout(waiter.timer);waiter.reject(new Error('approved-reference shutdown'));}
  pending.clear();
