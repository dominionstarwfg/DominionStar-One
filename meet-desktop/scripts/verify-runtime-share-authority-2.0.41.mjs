...
assert(intelligence.includes('async function recheckAfterSettings()')&&intelligence.includes('if(!settingsOpened)return;')&&!intelligence.includes('if(!settingsOpened||!recovery||recovery.hidden)return;'),'Settings-return recheck must run even when the native macOS permission prompt hid DominionStar recovery.');
assert(intelligence.includes('const box=ensureRecovery();')&&intelligence.includes('box.hidden=false;'),'A stale current process must surface the one-time restart path after returning from Settings instead of silently missing the new grant.');
...
console.log('DOMINIONSTAR_RUNTIME_SHARE_AUTHORITY_2_0_41_OK one-primary-share-command permission-before-enumeration no-stacked-tcc-dialog hidden-recovery-settings-recheck automatic-grant-recheck compact-prejoin approved-screens-files-more real-source-bridge no-second-capture-owner home-locked');
