...
try{
  ...
  // The old physical tray is still the polished reaction surface, but 2.0.22
  // deliberately removes its duplicate Raise Hand section because Raise Hand is
  // now a permanent first-class toolbar control.
  const reaction=await evaluate(`(()=>{const tray=document.createElement('div');tray.className='ds-reaction-tray';for(const e of ['👏','👍','❤️','😂','😮','🎉']){const b=document.createElement('button');b.textContent=e;tray.append(b);}const d=document.createElement('span');d.className='ds-reaction-divider';tray.append(d);const hand=document.createElement('button');hand.className='ds-raise-hand';hand.textContent='✋ Raise Hand';tray.append(hand);document.body.append(tray);const visible=[...tray.querySelectorAll('button:not(.ds-raise-hand)')].filter(b=>getComputedStyle(b).display!=='none');const out={reactions:visible.length,handHidden:getComputedStyle(hand).display==='none',dividerHidden:getComputedStyle(d).display==='none',overflow:getComputedStyle(tray).overflow,buttonWidth:visible[0]?.getBoundingClientRect().width||0,font:visible[0]?parseFloat(getComputedStyle(visible[0]).fontSize):0};tray.remove();return out;})()`);
  assert.equal(reaction.reactions,6,'Physical reaction tray must retain six standard reactions.');
  assert.equal(reaction.handHidden,true,'Legacy reaction-tray Raise Hand must stay hidden in 2.0.22.');
  assert.equal(reaction.dividerHidden,true,'Unused reaction-tray divider must stay hidden with the legacy hand section.');
  ...
  // video tile/body area, not only on a special header/grip. Establish Speaker
  // view after parity installation so a View mode persisted by an earlier gate
  // cannot contaminate this threshold test.
  const videoReady=await evaluate(`(()=>{document.querySelector('#bootScreen').hidden=true;document.querySelector('#authGate').hidden=true;document.querySelector('#appShell').hidden=true;document.querySelector('#prejoinOverlay').hidden=true;document.querySelector('#waitingOverlay').hidden=true;const overlay=document.querySelector('#meetingOverlay');overlay.hidden=false;window.DominionMeetingParity?.install?.();overlay.dataset.viewMode='speaker';overlay.classList.remove('share-active');document.body.classList.remove('remote-share-active');window.DominionMeetingParity?.syncVideoDock?.();const roster=document.querySelector('#participantRoster');roster.innerHTML='<div data-participant-id="self"></div><div data-participant-id="p2"></div><div data-participant-id="p3"></div>';const dock=document.querySelector('#participantVideoDock');const body=dock?.querySelector('.participant-video-dock-body');if(!dock||!body)return false;body.querySelectorAll('.remote-peer-tile:not(#localVideoDockTile)').forEach(n=>n.remove());for(const id of ['tile-a','tile-b']){const tile=document.createElement('article');tile.id=id;tile.className='remote-peer-tile';tile.style.minHeight='76px';tile.innerHTML='<div class="remote-peer-fallback"><span>'+id+'</span></div>';body.append(tile);}window.DominionPhysicalMacRepair.syncVideoDockPolicy();window.DominionZoomAdaptiveParity.installVideoDockDrag();return !dock.hidden&&dock.dataset.zoomThreshold==='available'&&dock.dataset.dsAdaptiveWholePanelDrag==='1';})()`);
  ...
  console.log('DOMINIONSTAR_PACKAGED_PHYSICAL_MAC_2_0_21_OK repair-loaded adaptive-loaded native-share-entry recovery-not-preemptive adhoc-not-certified actual-personal-ui-authority reaction-six-only settings-aligned actual-participant-count deterministic-speaker-threshold video-tile-real-mouse-drag normal-arrow-no-grip');
