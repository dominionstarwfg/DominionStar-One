...
assert(macToolbar.includes("if(rendererBridge?.command)return await sendRenderer(normalized);")&&macToolbar.includes("normalized!=='show-meeting'&&nativeBridge?.command"),'Physical-Mac functional presenter controls must execute through the authoritative renderer dispatcher, reserving the native bridge for dock layout only.');
...
assert(macPresenter.includes('const displayForSharedContent=()=>')&&macPresenter.includes('bounds=display.bounds,t=BORDER_THICKNESS,inset=2')&&macPresenter.includes('{x,y:y+height-t,width,height:t}')&&macPresenter.includes('setImmediate(()=>{if(shareActive&&bordersReady()){positionBorder();'),'The green presenter border must use the selected display and keep every edge inside the physical display boundary.');
assert(shareService.includes('main.setOpacity?.(0.02)')&&bootstrap.includes('originalSetOpacity.call(main,0.02)')&&macPresenter.includes('main.setOpacity?.(0.02)'),'The live meeting renderer must use one non-suspending Mac parking opacity across every presenter authority.');
...
assert(macMirror.includes("ipcMain.on('mac-share:camera-frame'")&&macMirror.includes("event.sender!==main.webContents"),'Floating presenter video must receive camera frames only from the main meeting renderer.');
...
console.log('DOMINIONSTAR_PHYSICAL_MAC_PARITY_2_0_44_OK authoritative-presenter-dispatch synchronized-media-ui live-camera-frame-bridge inset-share-border enlarged-profile-scale stable-right-panels mac-panel-controls canonical-participant-row');
