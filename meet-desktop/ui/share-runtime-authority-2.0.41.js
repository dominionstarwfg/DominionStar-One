(()=>{
  ...
  function ensureRoot(){
    ...
    root.querySelectorAll('[data-layout]').forEach(button=>button.onclick=()=>{if(button.disabled)return;root.querySelectorAll('[data-layout]').forEach(item=>item.classList.toggle('active',item===button));});
