...
try{
  ...
  await waitFor("(()=>{const menu=document.querySelector('.meeting-reaction-menu');const button=menu?.querySelector('.reaction-emoji-button');if(!menu||!button)return false;const mr=menu.getBoundingClientRect(),br=button.getBoundingClientRect();return mr.width>0&&mr.height>0&&br.width>0&&br.height>0;})()",'laid-out canonical reaction menu');
