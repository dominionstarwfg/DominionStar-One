...
import { resolveCaptureSource } from './capture-source.mjs';
...
function installPermissionPolicy(ses) {
  ...
  ses.setDisplayMediaRequestHandler(async (request, callback) => {
    ...
    try {
      ...
      lastCaptureFailure='';
      callback({video:source,...(selection.audio&&process.platform==='win32'?{audio:'loopback'}:{})});
      ...
      setTimeout(()=>mainWindow?.setContentProtection?.(selection.kind==='screen'),0);
    } catch (error) {
...
ipcMain.handle('desktop:select-share-source', (event, selection={}) => {
  ...
  const selected=captureSession.select(event.sender.id,{...selection,audio:Boolean(selection.audio)&&process.platform==='win32'});
...
ipcMain.handle('desktop:capture-status', event => {
  ...
  return {ok:screenStatus==='granted',screen:screenStatus,systemAudio:process.platform==='win32',platform:process.platform,lastFailure:lastCaptureFailure};
});
