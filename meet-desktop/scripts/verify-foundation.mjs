...
assert(js.includes("if(button.dataset.action==='share-screen')"),'Share control liveness placeholder is missing.');
assert(!html.includes('meet_personal_rooms'),'Undeployed backend table name must never leak into the Home UI.');
console.log('DOMINIONSTAR_DESKTOP_FOUNDATION_OK local-home four-primary-actions clean-settings responsive-controls');
