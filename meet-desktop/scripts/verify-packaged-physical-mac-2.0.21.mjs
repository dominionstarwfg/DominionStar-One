...
try{
  ...
  await sleep(80);
  const dragStart=await evaluate(`(()=>{const dock=document.querySelector('#participantVideoDock'),tile=dock.querySelector('#tile-a'),stage=document.querySelector('.stage'),dr=dock.getBoundingClientRect(),sr=stage.getBoundingClientRect(),tr=tile.getBoundingClientRect();return {left:dr.left,top:dr.top,rightGap:Math.round(sr.right-dr.right),vertical:Math.abs(tr.width-176)<28,x:tr.left+Math.max(8,Math.min(tr.width-8,tr.width*.5)),y:tr.top+Math.max(8,Math.min(tr.height-8,tr.height*.5)),cursor:getComputedStyle(tile).cursor,grip:getComputedStyle(dock.querySelector('.dock-grip')).display};})()`);
  ...
  assert.equal(dragStart.vertical,true,'Default desktop participant video filmstrip must use the vertical Zoom-style tile width.');
