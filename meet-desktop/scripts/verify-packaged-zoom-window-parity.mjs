...
try{
  ...
  // retain the normal arrow cursor.
  const dragStart=await evaluate(`(()=>{const panel=document.querySelector('.room-side'),head=panel.querySelector('.room-side-head'),r=panel.getBoundingClientRect(),h=head.getBoundingClientRect();return {left:r.left,top:r.top,x:h.left+Math.min(100,h.width*.42),y:h.top+h.height/2};})()`);
  ...
  assert.ok(Math.abs(dragged.dx)>=25||Math.abs(dragged.dy)>=18,'Floating participant panel must move using real mouse input.');
