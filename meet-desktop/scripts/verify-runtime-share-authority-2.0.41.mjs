...
assert(controller.includes("layout==='background'")&&controller.includes("layout==='shoulder'")&&controller.includes("drawContained(ctx,video,contentX,gap,contentW,height-(gap*2))"),'Share compositor must implement distinct background, shoulder, and side-by-side participant frames.');
