...
assert.equal(session.select(7,{sourceId:'screen:1:0',kind:'screen',displayId:'44',audio:true,shareOwnWindow:false}),true);
const first=session.consume(7);
assert.equal(first.sourceId,'screen:1:0');
assert.equal(first.shareOwnWindow,false);
