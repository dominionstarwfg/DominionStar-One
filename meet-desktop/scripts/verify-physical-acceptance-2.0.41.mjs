...
assert(intelligence.includes('async function commitGrantedShare(button)')&&intelligence.includes('deferPresenterCommit:true'),'A granted physical-Mac share must use the two-phase capture/presenter handoff.');
const physicalCommit=intelligence.slice(intelligence.indexOf('async function commitGrantedShare(button)'),intelligence.indexOf('async function open(){'));
assert(physicalCommit.indexOf('legacy.close?.()')>=0&&physicalCommit.indexOf('legacy.close?.()')<physicalCommit.indexOf('desktop.sharePicker?.choose?.(sourceId,options)'),'The pre-share chooser must be gone before display capture can transition the meeting renderer.');
assert(physicalCommit.includes('window.DominionShareIntegration?.state?.()?.active')&&physicalCommit.includes('window.DominionShareIntegration?.commitPresenterMode?.()'),'Physical presenter mode must begin only after real live capture is confirmed.');
...
assert(shareService.includes('function parkMacMeetingWindow({preCapture=false}={})')&&shareService.includes('parkMacMeetingWindow({preCapture:true})'),'The main macOS meeting window must have an explicit pre-capture preparation phase.');
assert(shareService.includes('if(preCapture){')&&shareService.includes('macPresenterParked=false')&&shareService.includes('meetingVisible:true'),'Pre-capture preparation must not park the meeting before getDisplayMedia finishes.');
assert(shareService.includes('main.setOpacity?.(1)')&&!shareService.includes('main.setOpacity?.(0.02)'),'Physical presenter mode must never regress to near-zero-opacity parking.');
assert(shareService.includes('main.webContents?.setBackgroundThrottling?.(false)')&&!shareService.includes('try{main.hide();}catch{}'),'The share service must keep the capture renderer scheduled and leave physical parking to the native overlay.');
assert(boot.includes('disable-backgrounding-occluded-windows')&&!boot.includes('try{main.hide();}catch{}'),'Bootstrap must not hide the capture renderer after native presenter parking is established.');
assert(macOverlay.includes('function parkCaptureOwnerBehindVideoDock')&&macOverlay.includes('screen.getAllDisplays?.()')&&macOverlay.includes('maxRight+96')&&macOverlay.includes('main.setAlwaysOnTop(false)'),'Physical Mac presenter mode must keep the exact capture owner composited off-display at full meeting size instead of shrinking it beneath the video dock.');
assert(macOverlay.includes('presenterRendererResponsive')&&macOverlay.includes("ipcMain.on('share:presenter-committed'"),'Native presenter chrome must wait for a post-capture renderer-liveness gate before becoming interactive.');
...
assert(macOverlay.includes("setAlwaysOnTop(true,'screen-saver',1)")&&macOverlay.includes('borderWindows.length===1'),'The green share boundary must use one full-display overlay so all four edges and corners stay aligned.');
assert(macOverlay.includes('captureOwnerWebContents')&&macOverlay.includes('executePresenterCommandDirect'),'Physical presenter commands must bind to the exact capture owner and retain direct execution authority.');
...
console.log('DOMINIONSTAR_PHYSICAL_ACCEPTANCE_2_0_41_OK working-view working-host-tools working-more participant-media participant-ellipsis modern-chat single-owner-clickable-reactions six-second-float canonical-presenter-new-share real-source-recheck inline-macos-permission-recovery identity-aware-tcc-reset two-phase-zoom-handoff off-display-capture-renderer fail-closed-stop exact-capture-owner single-overlay-full-display-border local-video-layout readable-settings');
