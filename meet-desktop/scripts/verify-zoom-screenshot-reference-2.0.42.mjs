...
assert.equal(pkg.version,'2.0.42','Zoom screenshot reference lock must run only on the 2.0.42 repair candidate.');
...
console.log('DOMINIONSTAR_ZOOM_SCREENSHOT_REFERENCE_2_0_42_OK home active-meeting-home prejoin meeting-toolbar participants participant-more host-tools meeting-more truthful-disabled-capabilities zoom-preshare bounded-share mouse-reveal-presenter mac-share-time-presenter green-share-boundary acknowledged-native-first-mac-presenter-routing isolated-presenter-preload vector-stop-share privacy');
