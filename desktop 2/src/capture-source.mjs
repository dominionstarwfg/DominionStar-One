...
export function isDominionStarCaptureSource(source={},ownSourceId='') {
  const id=String(source.id||'');
  const name=normalizedName(source.name);
  return Boolean(
    (ownSourceId&&id===String(ownSourceId))
    ||/\bdominionstar\s+meet\b/.test(name)
  );
}

export function visibleCaptureSources(sources=[],{includeOwnWindows=false,ownSourceId=''}={}) {
  const list=Array.isArray(sources)?sources:[];
  return list.filter(source=>includeOwnWindows||!isDominionStarCaptureSource(source,ownSourceId));
}
