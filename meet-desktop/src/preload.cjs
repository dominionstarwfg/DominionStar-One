...
contextBridge.exposeInMainWorld('dominionDesktop',Object.freeze({
  ...
  meeting:Object.freeze({
    ...
    requestJoin:input=>invoke('meeting:request-join',input),
    ...
    hostQueue:roomId=>invoke('meeting:host-queue',{roomId}),
    decide:(participantId,decision)=>invoke('meeting:decide',{participantId,decision}),
    snapshot:roomId=>invoke('meeting:snapshot',{roomId}),
    end:roomId=>invoke('meeting:end',{roomId})
  })
}));
