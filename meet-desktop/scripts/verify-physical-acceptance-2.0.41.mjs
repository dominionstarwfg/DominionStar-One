...
assert(shareService.includes('main.webContents?.setBackgroundThrottling?.(false)')&&shareService.includes('try{main.hide();}catch{}'),'Zoom-style presenter mode must hide the meeting window while keeping its capture renderer scheduled.');
assert(boot.includes('disable-backgrounding-occluded-windows')&&boot.includes('try{main.hide();}catch{}'),'The physical Mac bootstrap must keep hidden capture renderers active without leaving a compact Meet tile on the shared stage.');
