...
for(const rpc of ['meet_v2_create_room','meet_v2_get_personal_room','meet_v2_update_personal_room','meet_v2_start_personal_room','meet_v2_schedule_meeting','meet_v2_list_host_schedules','meet_v2_mark_schedule_started','meet_v2_cancel_schedule','meet_v2_request_join','meet_v2_join_status','meet_v2_mark_joined','meet_v2_leave_room','meet_v2_host_queue','meet_v2_decide_participant','meet_v2_room_snapshot','meet_v2_set_cohost','meet_v2_remove_participant','meet_v2_rename_participant','meet_v2_set_security','meet_v2_transfer_host_and_leave','meet_v2_end_room'])assert(service.includes(rpc),`Missing meeting RPC ${rpc}`);
...
for(const channel of ['meeting:create','meeting:personal-room','meeting:update-personal-room','meeting:start-personal-room','meeting:schedule','meeting:list-schedules','meeting:start-schedule','meeting:cancel-schedule','meeting:request-join','meeting:join-status','meeting:mark-joined','meeting:leave','meeting:host-queue','meeting:decide','meeting:snapshot','meeting:set-cohost','meeting:remove-participant','meeting:rename-participant','meeting:set-security','meeting:transfer-host-and-leave','meeting:end'])assert(main.includes(channel),`Missing IPC owner ${channel}`);
for(const method of ['create:','personalRoom:','updatePersonalRoom:','startPersonalRoom:','schedule:','listSchedules:','startSchedule:','cancelSchedule:','requestJoin:','joinStatus:','markJoined:','leave:','hostQueue:','decide:','snapshot:','setCohost:','removeParticipant:','renameParticipant:','setSecurity:','transferHostAndLeave:','end:'])assert(preload.includes(method),`Missing renderer bridge method ${method}`);
