...
try{
  ...
  await sleep(80);
  const reaction=await evaluate(`(()=>{const menus=[...document.querySelectorAll('.ds-reaction-tray,.meeting-reaction-menu')],menu=menus[0],r=menu?.getBoundingClientRect?.()||{left:0,bottom:innerHeight},anchor=document.querySelector('#roomReactions'),ar=anchor?.getBoundingClientRect?.()||{left:0,right:0,top:innerHeight,width:0},button=menu?.querySelector('button:not(.reaction-hand-button):not(.ds-raise-hand)'),br=button?.getBoundingClientRect?.(),cs=button?getComputedStyle(button):null,mcs=menu?getComputedStyle(menu):null;const ancestors=[];for(let node=button?.parentElement;node&&ancestors.length<6;node=node.parentElement){const s=getComputedStyle(node);ancestors.push({tag:node.tagName,class:node.className||'',display:s.display,visibility:s.visibility,opacity:s.opacity});}return {menuCount:menus.length,menuClass:menu?.className||'',menuConnected:Boolean(menu?.isConnected),menuDisplay:mcs?.display||'',menuVisibility:mcs?.visibility||'',menuOpacity:mcs?.opacity||'',menuWidth:r?.width||0,menuHeight:r?.height||0,buttonClass:button?.className||'',buttonConnected:Boolean(button?.isConnected),buttonDisplay:cs?.display||'',buttonVisibility:cs?.visibility||'',buttonOpacity:cs?.opacity||'',hasOffsetParent:Boolean(button?.offsetParent),above:r.bottom<=ar.top+2,anchorDelta:Math.round(Math.abs((r.left+r.width/2)-(ar.left+ar.width/2))),buttonWidth:br?.width||0,buttonHeight:br?.height||0,font:cs?parseFloat(cs.fontSize):0,computedWidth:cs?.width||'',computedMinWidth:cs?.minWidth||'',computedTransform:cs?.transform||'',inlineStyle:button?.getAttribute('style')||'',ancestors};})()`);
  console.log('REACTION_VISUAL_DIAGNOSTIC',JSON.stringify(reaction));
  assert.equal(reaction.above,true,'Reactions tray must open directly above the React toolbar control.');
  assert.ok(reaction.anchorDelta<=18,'Reactions tray must stay centered over the React toolbar control.');
  ...
  console.log('DOMINIONSTAR_PACKAGED_ZOOM_VISUAL_OK stable-three-zone-toolbar reference-footer-56 icons-21 labels-10 audio-left video-left actions-centered end-right share-green host-tools participants-right-stable compact-reference-text reactions-above-control more-reference-text no-uncaught-renderer-errors');
