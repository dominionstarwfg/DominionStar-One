(()=>{
  ...
  async function start({name='',options={}}={}){
    ...
    try{
      ...
      let presenter=null;
      try{
        const acknowledgement=Promise.resolve(bridge?.captureStarted?.({sourceName:state.sourceName,sourceId:String(state.options?.sourceId||''),sourceKind:String(state.options?.sourceKind||''),displayId:String(state.options?.displayId||''),paused:false,shareAudio:Boolean(state.options?.shareAudio),shareAudioMode:normalizeAudioMode(state.options?.shareAudioMode),optimizeVideo:Boolean(state.options?.optimizeVideo),presenterLayout:normalizePresenterLayout(state.options?.presenterLayout),showGreenBorder:state.options?.showGreenBorder!==false,includeMeetWindows:Boolean(state.options?.includeMeetWindows)}));
  ...
  async function replaceSource({name='',options={}}={}){
    ...
    try{
      ...
      try{await bridge?.captureState?.({sourceName:state.sourceName,sourceId:String(state.options?.sourceId||''),sourceKind:String(state.options?.sourceKind||''),displayId:String(state.options?.displayId||''),paused:false,shareAudio:Boolean(state.options?.shareAudio),shareAudioMode:normalizeAudioMode(state.options?.shareAudioMode),optimizeVideo:Boolean(state.options?.optimizeVideo),presenterLayout:normalizePresenterLayout(state.options?.presenterLayout),showGreenBorder:state.options?.showGreenBorder!==false,includeMeetWindows:Boolean(state.options?.includeMeetWindows)});}catch{}
      return snapshot();
    }catch(error){
