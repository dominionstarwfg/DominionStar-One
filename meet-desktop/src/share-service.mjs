...
export function createShareService({BrowserWindow,desktopCapturer,desktopSession,ipcMain,path,uiDir,preloadPath,getMainWindow,platform,ensureScreenPermission,openPrivacySettings}){
  ...
  function restoreMainWindowAfterShare(){
    ...
    if(platform==='darwin'){try{main.setVisibleOnAllWorkspaces(false);}catch{}}protectMeetingChrome(main,false);try{main.webContents?.setBackgroundThrottling?.(false);}catch{}
    try{main.show();main.moveTop?.();main.focus();}catch{try{main.show();}catch{}}
  ...
  ipcMain.handle('share:select-source',(_event,{sourceId,options={}}={})=>{configureDisplayMediaHandler(false);
    ...
    const rawWallpaper=String(options.presenterWallpaper||''),presenterWallpaper=/^data:image\/(?:png|jpeg|webp);base64,/i.test(rawWallpaper)&&rawWallpaper.length<=3500000?rawWallpaper:'';
    const normalizedOptions={presenterLayout,presenterGeometry,presenterWallpaper,optimizeVideo:Boolean(options.optimizeVideo),shareAudio:Boolean(options.shareAudio),shareAudioMode:String(options.shareAudioMode||'mono')==='stereo'?'stereo':'mono',showGreenBorder:options.showGreenBorder!==false,includeMeetWindows:Boolean(options.includeMeetWindows),sourceKind,displayId};pendingSelection={source,options:normalizedOptions};
