...
// Physical Mac repair: use live capture access, not only a stale TCC status string.
...
assert(intelligence.includes('async function liveScreenAccess()')&&intelligence.includes('desktop.media.requestScreen()'),'Share must probe the real bounded macOS screen-capture path before deciding access is unavailable.');
assert(intelligence.includes("desktop.media?.permissions?.()")&&intelligence.includes("ready=Boolean(diagnostic?.ok)||status==='granted'"),'Live capture proof must override a stale raw TCC status when the screen is actually readable.');
assert(intelligence.includes("detectedBy:'renderer-timeout'")||intelligence.includes("'renderer-timeout'"),'Screen access probing must be bounded so Share cannot hang indefinitely.');
assert(intelligence.includes('.ds2041-smart-recovery{')&&intelligence.includes('z-index:9800!important')&&intelligence.includes('.ds2041-smart-recovery[hidden]{display:none!important}'),'Permission recovery must own visible fixed styling and cannot depend on the not-yet-open picker stylesheet.');
assert(intelligence.includes("title.textContent='Checking Screen Recording…'")&&intelligence.includes("copy.textContent='DominionStar Meet is checking the live macOS capture state.'"),'Every Share click must provide immediate visible permission-check feedback instead of appearing inert.');
...
assert(intelligence.includes('async function recheckAfterSettings()')&&intelligence.includes('if(!settingsOpened)return;'),'Settings-return recheck must run after the user changes Screen Recording access.');
assert(intelligence.includes('const state=await liveScreenAccess();')&&intelligence.includes('if(state.ready)'),'Settings return must use the same real access probe as the initial Share click.');
assert(intelligence.includes("copy.textContent='DominionStar Meet can read your screen. Opening the approved Screens / Files / More chooser…'")&&intelligence.includes('return await legacy.open()'),'A usable permission must automatically continue into the approved chooser without another user action.');
assert(intelligence.includes("copy.textContent='macOS has not exposed the new Screen Recording grant to this running process yet."),'A stale current process must be identified as a one-time restart condition rather than another grant request.');
...
console.log('DOMINIONSTAR_RUNTIME_SHARE_AUTHORITY_2_0_41_OK one-primary-share-command live-capture-permission-probe stale-tcc-override bounded-share-entry visible-recovery automatic-settings-recheck compact-prejoin approved-screens-files-more real-source-bridge no-second-capture-owner home-locked');
