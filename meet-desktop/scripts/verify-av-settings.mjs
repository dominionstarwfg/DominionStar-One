...
assert(enhancements.includes('startSettingsPreview')&&enhancements.includes('media.testCameraStream')&&enhancements.includes('stopSettingsPreview'),'Video Settings must start and clean up an isolated preview without changing meeting camera state.');
assert(!enhancements.includes('media.setCamera(true)'),'Opening Video Settings must never turn the participant-facing meeting camera on.');
assert(enhancements.includes("dialog.classList.add('av-video-settings-open')")&&css.includes('.settings-modal.av-video-settings-open')&&css.includes('.zoom-reference-preview{height:300px'),'Video Settings must expand into a desktop-scale layout with a large Zoom-style preview.');
...
console.log('DOMINIONSTAR_AV_SETTINGS_PARITY_OK private-video-preview expanded-video-panel zoom-settings-hierarchy devices device-tests audio-processing original-sound quick-menus mirror original-ratio low-light-auto-manual touch-up portrait-lighting auto-framing background-blur virtual-backgrounds feathered-foreground adaptive-performance video-denoise processed-outgoing-track quality clean-media-authority');
