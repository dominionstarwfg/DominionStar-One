...
assert(parity.includes("if(key===toolbarOrderKey&&visible===key)return"),'Primary toolbar ordering must be idempotent so reconciliation cannot make controls dance.');
assert(parity.includes("overlay.dataset.dsParityInstalled='1'"),'Meeting parity must mark the live shell installed instead of rebuilding it.');
assert(parity.includes("if(share){syncShareLayout();return;}"),'Active screen sharing must update Zoom-style layout before any participant media dock rebinding can occur.');
assert(parity.includes("window.addEventListener('dominion:meeting-snapshot',scheduleParityRefresh)")&&parity.includes("window.addEventListener('dominion:participant-presence',scheduleParityRefresh)")&&parity.includes("window.addEventListener('dominion:preference-change',scheduleParityRefresh)"),'Meeting parity must refresh from authoritative meeting, participant, and preference events.');
assert(!parity.includes("setInterval(()=>{syncGreeting();"),'Meeting parity must not run a legacy periodic UI/media reconciliation loop.');
...
assert(parity.includes("const desired=['roomMic','roomCamera','roomShare','roomParticipants','roomChat','roomReactions','roomMore','roomExitButton']"),'Primary toolbar order must stay Mic, Video, Share, Participants, Chat, Reactions, More, End/Leave.');
...
assert(participantControls.includes('Spotlight for Everyone')&&participantControls.includes('Add Spotlight')&&parity.includes('spotlightParticipantIds')&&parity.includes("spotlightTile||q('#remoteTileStrip .remote-peer-tile.active-speaker')"),'Spotlight must override active-speaker promotion through the meeting stage authority and support an ordered multi-spotlight set.');
assert(parity.includes("lastMeta='',spotlightParticipantIds=[]"),'Meeting stage must own initialized spotlight-set state before any render or dock synchronization runs.');
