(()=>{
  ...
  function restoreControls(){for(const tab of $$('.tab'))tab.disabled=false;$('#optimizeVideo').disabled=false;const audio=$('#shareAudio');if(audio&&!audio.closest('.option-row')?.classList.contains('disabled'))audio.disabled=false;shareButton.disabled=!selectedId;}
  function render(){
  ...
  async function refresh(){
    ...
    try{
      const result=await bridge?.listSources?.({includeDominionStar:false,kind:filter==='window'?'window':'screen'});
  ...
  shareButton.addEventListener('click',async()=>{
    ...
    try{
      try{localStorage.setItem('ds_pref_share_optimize',$('#optimizeVideo').checked?'1':'0');localStorage.setItem('ds_pref_share_audio',$('#shareAudio').checked?'1':'0');}catch{}
  ...
  window.dominionDesktop?.environment?.().then(info=>{const platform=String(info?.platform||'');const supported=platform==='win32'||platform==='darwin';$('#shareAudio').disabled=!supported;$('#shareAudioRow').classList.toggle('disabled',!supported);$('#audioSupportCopy').textContent=supported?'Include computer audio when the operating system permits it.':'Computer audio sharing is unavailable on this desktop platform.';}).catch(()=>{$('#shareAudio').disabled=true;$('#shareAudioRow').classList.add('disabled');});
  void refresh();
})();
