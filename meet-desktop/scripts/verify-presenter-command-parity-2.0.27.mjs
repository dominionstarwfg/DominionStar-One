...
assert.ok(shareIntegration.includes("if(command==='stop'){clearCompanion();await share.stop();return {handled:true,command};}")&&shareIntegration.includes("share.onChange(state=>{applyLayout();")&&shareIntegration.includes("DominionActiveShareHomeParity2041?.restoreMeeting?.()"),'Certified Stop Share authority must terminate capture locally, restore layout through the single share-state listener, and return to the same meeting.');
