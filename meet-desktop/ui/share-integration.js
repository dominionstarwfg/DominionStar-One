  ...
  async function boot(){
    ...
    async function dispatchPresenterCommand(rawCommand){
      ...
      try{
        ...
        if(command.startsWith('presenter-layout-')){
          const mode=command.replace('presenter-layout-','');await share.setPresenterLayout?.(mode);applyLayout();return {handled:true,command};
        }
        if(command.startsWith('presenter-geometry:')){
          try{const geometry=JSON.parse(decodeURIComponent(command.slice('presenter-geometry:'.length)));await share.setPresenterGeometry?.(geometry);applyLayout();return {handled:true,command};}
          catch(error){return {handled:false,command,error:String(error?.message||error||'invalid_presenter_geometry')};}
        }
