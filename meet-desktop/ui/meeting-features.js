(()=>{
  ...
  async function refreshChatRecipients(){const select=q('#meetingChatRecipient');if(!select)return;const current=select.value||'everyone';let people=[];try{people=await peers();}catch{}const options=['<option value="everyone">Everyone</option>',...people.map(p=>`<option value="${esc(String(p.participantId||''))}">${esc(String(p.displayName||p.name||'Participant'))} · Direct Message</option>`)];select.innerHTML=options.join('');if([...select.options].some(o=>o.value===current))select.value=current;}
  function syncChatUnread(){const button=q('#roomChat');if(!button)return;button.dataset.unread=String(state.chatUnread);button.classList.toggle('has-unread',state.chatUnread>0);button.setAttribute('aria-label',state.chatUnread>0?`Chat, ${state.chatUnread} unread message${state.chatUnread===1?'':'s'}`:'Chat');}
  function toggleChat(force){ensureUi();const panel=q('#meetingChatPanel'),button=q('#roomChat');if(!panel)return;const show=typeof force==='boolean'?force:panel.hidden;panel.hidden=!show;button?.setAttribute('aria-pressed',String(show));if(show){state.chatUnread=0;syncChatUnread();void refreshChatRecipients();requestAnimationFrame(()=>q('#meetingChatInput')?.focus());}return show;}
  ...
  function appendMessage(message){state.messages.push(message);renderMessages();if(!message.own&&q('#meetingChatPanel')?.hidden){state.chatUnread=Math.min(99,state.chatUnread+1);syncChatUnread();window.DominionMeetingNotifications?.chat?.(message.name||'Participant');}}
  ...
  window.addEventListener('dominion:meeting-snapshot',event=>{
    state.meetingSnapshot=event.detail||null;void refreshChatRecipients();
  ...
  ensureUi();
  window.DominionMeetingFeatures=Object.freeze({version:'1.4.0-zoom-chat',toggleChat,openReactions,sendReaction,toggleRaiseHand,setLocalHand,lowerParticipantHand,toggleRecording,stopRecording,setVideoLayout,refreshChatRecipients,snapshot:()=>({chatOpen:!q('#meetingChatPanel')?.hidden,chatUnread:state.chatUnread,recording:state.recording,recordingPaused:state.recordingPaused,messageCount:state.messages.length,handRaised:state.localHandRaised,raisedHands:[...state.raisedHands.keys()],reactions:[...state.reactions.entries()].map(([participantId,value])=>({participantId,...value}))})});
})();
