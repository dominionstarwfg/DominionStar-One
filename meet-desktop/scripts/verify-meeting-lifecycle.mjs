...
assert(migration.includes('recording_allowed boolean not null default false')&&migration.includes('is_recording boolean not null default false'),'Recording permission and active state must be participant-scoped server state.');
...
assert(migration.includes("'recordingAllowed',p.recording_allowed,'isRecording',p.is_recording"),'Room snapshots must expose recording permission and active state for late joiners.');
