...
// Modern macOS uses one Electron handler with Apple's native picker. Fallback
// source enumeration stays bounded for older macOS and Windows only.
assert.match(main,/function supportsMacSystemPicker\(\)/,'native Mac picker capability check is missing');
assert.match(main,/major >= 15/,'native Mac picker must target macOS 15+');
...
assert.match(nativeCapture,/supportsNativeMacPicker/,'renderer native-picker capability is missing');
...
assert.match(preload,/let shareSourcesInFlight = null/,'fallback source enumeration must be serialized');
...
console.log('DOMINIONSTAR_CLEAN_SINGLE_AUTHORITY_CONTRACT_OK one-home settings-owned native-mac-picker');