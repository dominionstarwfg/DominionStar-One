...
for(const key of ['joinMuted','joinVideoOff','showJoinPreview','hideSelfView','waitingRoomSound','joinLeaveSound','desktopMeetingNotifications','alwaysShowMeetingControls','shareVideoDock','shareSideBySide','shareOptimize','shareAudio','shareScaleToFit','shareEnterFullScreen','shareGreenBorder','chatSound','recordMic','recordRemote','uiScale','shortcuts','globalShortcutMute','globalShortcutVideo','globalShortcutShare','globalShortcutParticipants','globalShortcutChat','globalShortcutHand','captionFontSize','captionFontType','captionTheme','captionPosition','alwaysShowCaptions'])assert(preferences.includes(`${key}:`),`Preferences missing ${key}.`);
...
assert(meetingFeatures.includes("version:'1.4.0-zoom-chat-shell'")&&meetingFeatures.includes('recordingAuthority()'),'Recording must have a dedicated permission check before creating a local recording.');
...
assert(presenterHtml.includes('id="layoutMenu"')&&presenterHtml.includes('data-command="presenter-layout-content"')&&presenterHtml.includes('data-command="presenter-layout-background"')&&presenterHtml.includes('data-command="presenter-layout-shoulder"')&&presenterHtml.includes('data-command="presenter-layout-side"'),'Presenter Layout control must expose Content only, As background, Over the shoulder, and Side by side compositing choices.');
