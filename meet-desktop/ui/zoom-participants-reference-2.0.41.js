(()=>{
  ...
  function ensureStyle(){
    ...
    style.textContent=`
      /* DominionStar Meet 2.0.41 — Zoom-familiar Participants utility window authority. */
      ...
      #meetingOverlay .room-side.ds-participants-reference .room-side-head{
      ...
      #meetingOverlay .room-side.ds-participants-reference .zoom-participant-search{
        ...
        color:#f5f5f6!important;
        font:500 12px/35px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif!important;
        box-shadow:none!important;
      }
      #meetingOverlay .room-side.ds-participants-reference .zoom-participant-search:focus{border-color:#36a8ff!important;box-shadow:0 0 0 1px #36a8ff!important}
      #meetingOverlay .room-side.ds-participants-reference .zoom-participant-search::placeholder{color:#c6c6c8!important;opacity:1!important}
      ...
      #meetingOverlay .room-side.ds-participants-reference #participantRoster{
        ...
        scrollbar-color:#626367 transparent!important;
      }
      ...
      #meetingOverlay .room-side.ds-participants-reference #participantRoster .person-badge{
        width:34px!important;height:34px!important;border-radius:50%!important;margin:0!important;overflow:hidden!important;display:grid!important;place-items:center!important;background:#59616d!important;color:#fff!important;font-size:11px!important;font-weight:700!important
      }
      ...
      #meetingOverlay .room-side.ds-participants-reference #participantRoster .participant-media-icon{width:18px!important;height:18px!important;display:grid!important;place-items:center!important;color:#d5d5d7!important}
      #meetingOverlay .room-side.ds-participants-reference #participantRoster .participant-media-icon svg{width:18px!important;height:18px!important;fill:none!important;stroke:currentColor!important;stroke-width:1.8!important}
      ...
      .ds-participant-share-state{width:19px;height:19px;display:grid;place-items:center;color:#26d26f;flex:none}
      .ds-participant-share-state svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.9}
      ...
      #meetingOverlay .room-side.ds-participants-reference #participantRoster .participant-more,#meetingOverlay .room-side.ds-participants-reference #participantRoster .ds-participant-more{width:24px!important;height:24px!important;min-width:24px!important;padding:0!important;border:0!important;border-radius:5px!important;background:transparent!important;color:#d7d7d9!important;font-size:0!important;display:grid!important;place-items:center!important}
      ...
      #meetingOverlay .room-side.ds-participants-reference #participantRoster .participant-more:hover,#meetingOverlay .room-side.ds-participants-reference #participantRoster .ds-participant-more:hover{background:#4a4a4d!important}
      #participantBulkActions{display:none!important}
      #meetingOverlay .room-side.ds-participants-reference .ds-ref-participants-footer{
        position:relative!important;left:auto!important;right:auto!important;bottom:auto!important;flex:0 0 58px!important;height:58px!important;padding:10px 12px!important;margin:0!important;border-top:1px solid #454649!important;border-radius:0 0 10px 10px!important;background:#333335!important;display:flex!important;align-items:center!important;gap:10px!important
      }
  ...
  function ensureHeader(side){
    ...
    if(!head){
      head=document.createElement('header');head.className='room-side-head';
      head.innerHTML='<div><strong>Participants</strong><small></small></div><button type="button" aria-label="Close participants">×</button>';
      side.prepend(head);
    }
    const close=head.querySelector('button');
    if(close&&close.dataset.dsParticipantRefClose!=='1'){
      close.dataset.dsParticipantRefClose='1';close.onclick=()=>q('#roomParticipants')?.click();
    }
    if(isMac&&!head.querySelector('.ds-participants-traffic')){
      side.classList.add('ds-participants-mac');
      const traffic=document.createElement('div');traffic.className='ds-participants-traffic';
      traffic.innerHTML='<button type="button" class="close" aria-label="Close participants"></button><button type="button" class="min" aria-label="Collapse participants"></button><button type="button" class="max" aria-label="Expand participants"></button>';
  ...
  function ensureSearch(side){
    ...
    if(!wrap){
      ...
      wrap.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="7"></circle><path d="m16.5 16.5 4 4"></path></svg><input class="zoom-participant-search" type="search" autocomplete="off" placeholder="Search" aria-label="Search participants">';
      const head=ensureHeader(side);head.insertAdjacentElement('afterend',wrap);
      const input=wrap.querySelector('input');input.addEventListener('input',()=>filterRows(input.value));
    }
    const input=wrap.querySelector('.zoom-participant-search');if(input)input.hidden=false;
    return input;
  }
  ...
  function filterRows(value){
    ...
    for(const row of participantRows()){
      const name=String(row.dataset.participantName||row.textContent||'').toLocaleLowerCase();
      row.hidden=Boolean(term&&!name.includes(term));if(!row.hidden)visible++;
    }
    const roster=q('#participantRoster');if(!roster)return;
    let empty=roster.querySelector('.ds-participant-search-empty');
    if(term&&visible===0){if(!empty){empty=document.createElement('div');empty.className='ds-participant-search-empty';empty.textContent='No participants found';roster.append(empty);}}
    else empty?.remove();
  }

  function micIsOn(row){return Boolean(row.querySelector('.participant-mic.on,.ds-media-state.on,[data-participant-mic].on'));}
  function sortRows(){
    const roster=q('#participantRoster');if(!roster)return;
    const rows=participantRows();
    const rank=row=>{
      const self=row.dataset.participantSelf==='1',role=String(row.dataset.participantRole||'participant').toLowerCase().replace('-','');
      if(self)return 0;if(role==='host')return 1;if(role==='cohost')return 2;if(micIsOn(row))return 3;return 4;
    };
  ...
  function syncShareState(){
    ...
    for(const row of participantRows()){
      ...
      if(active&&self){
        if(!icon){icon=document.createElement('span');icon.className='ds-participant-share-state';icon.title='Sharing screen';icon.setAttribute('aria-label','Sharing screen');icon.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="14" rx="2"></rect><path d="m8 11 4-4 4 4M12 7v8"></path></svg>';row.querySelector('.participant-media-state')?.prepend(icon);}
      }else icon?.remove();
    }
  }
  ...
  function saveGeometry(side){
    ...
    const body=q('#meetingOverlay .meeting-body');if(!body)return;
    const sr=side.getBoundingClientRect(),br=body.getBoundingClientRect();
    try{localStorage.setItem(GEOMETRY_KEY,JSON.stringify({left:Math.max(0,sr.left-br.left),top:Math.max(0,sr.top-br.top),width:sr.width,height:sr.height}));}catch{}
  }

  function restoreGeometry(side){
    if(side.dataset.dsParticipantsRefGeometry==='1')return;
    side.dataset.dsParticipantsRefGeometry='1';side.dataset.zoomPanelMode='popout';side.dataset.dsAdaptiveMode='floating';side.dataset.dsAdaptiveInitialized='1';
    const body=q('#meetingOverlay .meeting-body');if(!body)return;
    ...
    if(saved&&Number.isFinite(saved.left)&&Number.isFinite(saved.top)&&Number.isFinite(saved.width)&&Number.isFinite(saved.height)){
      const width=Math.min(Math.max(saved.width,320),Math.min(460,body.clientWidth-24));
      const height=Math.min(Math.max(saved.height,310),body.clientHeight-24);
      const left=Math.max(10,Math.min(saved.left,body.clientWidth-width-10));
      const top=Math.max(10,Math.min(saved.top,body.clientHeight-height-10));
      ...
      const width=Math.min(390,Math.max(320,body.clientWidth-24)),height=Math.min(520,Math.max(310,body.clientHeight-24));
      side.style.setProperty('width',`${width}px`,'important');side.style.setProperty('height',`${height}px`,'important');side.style.setProperty('left','auto','important');side.style.setProperty('right','12px','important');side.style.setProperty('top','12px','important');side.style.setProperty('bottom','auto','important');side.style.setProperty('transform','none','important');
    }
  }

  function syncFooter(side){
    const footer=side.querySelector('.ds-ref-participants-footer');if(!footer)return;
    const action=footer.querySelector('[data-ref-mute-all]');if(!action)return;
    const manager=['host','cohost'].includes(localRole());
    const desired=manager?'Mute all':'Unmute me';if(action.textContent!==desired)action.textContent=desired;
    if(action.dataset.dsParticipantRefAction!==desired){
      action.dataset.dsParticipantRefAction=desired;
      action.onclick=()=>{if(manager)void window.DominionParticipantControls?.sendAll?.('host:mute');else{const mic=q('#roomMic');if(mic?.classList.contains('is-off')||/unmute/i.test(String(mic?.getAttribute('aria-label')||'')))mic.click();}};
    }
  }

  function sync(){
    syncFrame=0;ensureStyle();if(!meetingOpen())return;
    const side=sidePanel();if(!side)return;
    side.classList.add('ds-participants-reference');ensureHeader(side);ensureSearch(side);restoreGeometry(side);
    ...
    sortRows();syncShareState();syncFooter(side);
    const search=side.querySelector('.zoom-participant-search');if(search)filterRows(search.value);
  }
  ...
  document.addEventListener('pointerup',()=>{const side=sidePanel();if(side&&!side.hidden)saveGeometry(side);},true);
  window.addEventListener('resize',schedule,true);
  window.addEventListener('dominion:remote-media-state',schedule,true);
