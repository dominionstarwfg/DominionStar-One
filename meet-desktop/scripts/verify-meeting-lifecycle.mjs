...
assert(migration.includes('meet_v2_set_caption_state')&&migration.includes("if coalesce(v_role,'') not in ('host','cohost')"),'Only host/co-host authority may configure manual captions or transcript retention.');
...
assert(migration.includes("'caption'"),'Authenticated signaling must explicitly allow live caption delivery.');
