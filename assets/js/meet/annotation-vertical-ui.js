(() => {
  ...
  style.textContent = `
    .ds-annotation-toolbar.ds-vertical-annotation-rail{
      ...
      background:rgba(8,13,21,.93)!important;
    ...
    .ds-vertical-annotation-rail button::after{
      ...
      font:700 11px/1.1 Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;
      box-shadow:0 8px 24px rgba(0,0,0,.35);
      transition:opacity .12s ease,transform .12s ease;
    }
    .ds-vertical-annotation-rail button:hover::after{opacity:1;transform:translateY(-50%) translateX(0)}
    @media(max-width:760px){
      .ds-annotation-toolbar.ds-vertical-annotation-rail{left:10px!important;transform:translateY(-50%) scale(.92)!important;transform-origin:left center!important}
    }
  `;
  document.head.append(style);

  const decorate = toolbar => {
    if (!toolbar || toolbar.dataset.verticalUi === '1') return false;
    ...
    [...toolbar.querySelectorAll('button')].forEach(button => {
      ...
      let key = raw;
      if (/^Clear/i.test(raw)) key = 'Clear';
      if (ICONS[key]) {
        button.setAttribute('aria-label', raw || key);
        button.setAttribute('title', raw || key);
  ...
  const refresh = () => {
    const toolbar = document.querySelector('.ds-annotation-toolbar');
  ...
  window.DominionVerticalAnnotationUI = Object.freeze({
    version: '1.0.0',
    refresh
  });
})();