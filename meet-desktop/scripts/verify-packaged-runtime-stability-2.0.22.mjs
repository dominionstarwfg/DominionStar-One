...
try{
  ...
  assert.ok(participantsImmediate.centerDelta<=8,'Participants must open at one stable centered floating position.');
  ...
  const dragMoved=await evaluate(`(()=>{const panel=document.querySelector('.room-side'),r=panel.getBoundingClientRect();let telemetry=null;try{telemetry=JSON.parse(panel.dataset.dsRuntimeDragTelemetry||'null');}catch{}return {left:Math.round(r.left),top:Math.round(r.top),pointerMoves:Number(panel.dataset.dsRuntimePointerMoves||0),mouseMoves:Number(panel.dataset.dsRuntimeMouseMoves||0),telemetry};})()`);
