...
try{
  ...
  assert.equal(raised.label,'Lower hand');assert.equal(raised.pressed,'true');assert.equal(raised.react,'React','Raising a hand must not rename the React control.');assert.ok(raised.reactFont>=11.5,'React label must remain readable while hand is raised.');
