...
assert(meetingCaptions.includes("version:'1.2.0'")&&meetingCaptions.includes("roomCaptions")&&meetingCaptions.includes("roomCaptionsMenu"),'Meeting shell must expose Zoom-style Show/Hide Captions plus a caption-options control.');
...
assert(meetingCaptions.includes('setCaptionState')&&meetingCaptions.includes('Select manual captioner')&&meetingCaptions.includes('Start Manual Captions')&&meetingCaptions.includes("const canManage=()=>state.role==='host'"),'Caption setup must be host-only and use server-backed manual-captioner authority.');
