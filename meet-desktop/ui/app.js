(async()=>{
  ...
  function renderRoster(people){
    ...
    $('#participantRoster').innerHTML=sorted.map(p=>{
      ...
      return `<div class="person-row" data-participant-id="${esc(p.participantId)}" data-participant-role="${esc(role)}" data-participant-name="${esc(name)}" data-recording-allowed="${p.recordingAllowed?'1':'0'}" data-record-eligible="${p.memberId?'1':'0'}" data-participant-self="${self?'1':'0'}"><span class="person-badge">${initials(name)}</span><span class="person-copy"><strong>${esc(name)}${self?' <em class="participant-you">(You)</em>':''}</strong><small>${role==='host'?'Host':role==='cohost'?'Co-host':'Participant'}</small></span><span class="participant-media-state" aria-label="Participant media status"><span class="participant-media-icon participant-mic unknown" data-participant-mic title="Microphone status">●</span><span class="participant-media-icon participant-video unknown" data-participant-video title="Video status">■</span></span><span class="participant-actions"></span></div>`;
