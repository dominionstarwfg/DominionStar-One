(()=>{
  ...
  // On macOS the floating presenter window is owned by the native macShare
  // overlay. Use that bridge first because it now requires an acknowledgement
  // from the live meeting renderer before reporting success. The generic
  // presenter bridge remains a bounded fallback only when native delivery fails.
  ...
  const send=async command=>{
    ...
    try{
      ...
      if(nativeBridge?.command){
        result=await nativeBridge.command(normalized);
        if(accepted(result))return result;
      }
      if(fallbackBridge?.command){
        result=await fallbackBridge.command(normalized);
  ...
  window.DominionMacPresenterToolbar=Object.freeze({
    version:'2.0.41-physical-command-ack',
    transport:nativeBridge?.command?'macShare-ack':fallbackBridge?.command?'presenter-fallback':'unavailable',
    state:()=>({...lastState})
  });
  reveal();scheduleHide();
})();
