...
assert(runtime.includes("handle.addEventListener('mousedown',event=>begin(event,'mouse'),true);")&&runtime.includes("document.addEventListener('mousemove',event=>move(event,'mouse'),true);")&&runtime.includes("document.addEventListener('mouseup',event=>end(event,'mouse'),true);"),'Final floating-panel drag authority must support both Pointer Events and Mac/Chromium mouse synthesis through the same geometry path.');
