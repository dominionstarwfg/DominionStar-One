...
const availableDisplaySources = new Map();
...
function isDesktopRoute(value) {
  try {
    ...
    return url.origin === APP_ORIGIN && ['/meet', '/meet-home', '/meet-login', '/member-login'].includes(route);
  } catch { return false; }
}
...
function installPermissionPolicy() {
  ...
  ses.setDisplayMediaRequestHandler(async (request, callback) => {
    ...
    try {
      ...
      const source = availableDisplaySources.get(selection.sourceId);
      if(source)activeShareDisplayId=String(source.display_id||selection.displayId||'');
      if (source) mainWindow?.setContentProtection?.(selection.kind === 'screen');
      callback(source ? { video: source, ...(selection.audio && process.platform === 'win32' ? { audio: 'loopback' } : {}) } : {});
    } catch {
      callback({});
    }
  });
...
ipcMain.handle('desktop:share-sources', async (event) => {
  ...
  availableDisplaySources.clear();
  sources.forEach(source=>availableDisplaySources.set(source.id,source));
...
ipcMain.handle('desktop:select-share-source', (event, selection={}) => {
  ...
  if(!sourceId||!availableDisplaySources.has(sourceId))return false;
  pendingDisplaySelections.set(event.sender.id,{sourceId,displayId:String(selection.displayId||''),audio:Boolean(selection.audio)&&process.platform==='win32',kind:String(selection.kind||'')});
  return true;
});
ipcMain.handle('desktop:end-share', event => {
  if(!isDesktopRoute(event.sender.getURL()))return false;
  activeShareDisplayId='';
