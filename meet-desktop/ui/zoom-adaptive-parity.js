(()=>{
  ...
  function syncParticipants(){
    ...
    if(!side.hidden&&!side.dataset.dsAdaptiveInitialized){
      ...
      // The physical Zoom reference supplied for a small participant roster is
      // a compact floating window. Start small meetings there; users can still
      // merge/dock through the panel-layout control.
      if(count<=6)centerParticipantPanel(side,count);
    }else if(!side.hidden&&side.dataset.dsAdaptiveMode==='floating'&&side.dataset.zoomPanelMode==='popout'){
      // Keep a floating panel inside the meeting bounds after resize/count change.
    ...
    for(const row of rows){
      row.querySelector('.ds-role-chip')?.setAttribute('aria-hidden','true');
    }
  }

  function chatOpen(){const panel=q('#meetingChatPanel');return Boolean(panel&&!panel.hidden);}
  ...
  function syncChat(){
    ...
    panel.style.setProperty('right',wide?'8px':'12px','important');
    panel.style.setProperty('left','auto','important');
    panel.style.setProperty('top',wide?'8px':'12px','important');
    panel.style.setProperty('bottom',wide?'8px':'auto','important');
    panel.style.setProperty('width',`${wide?340:Math.min(360,Math.max(290,body.clientWidth-24))}px`,'important');
    panel.style.setProperty('height',wide?'auto':`${Math.min(520,Math.max(340,body.clientHeight-24))}px`,'important');
  ...
  function ensurePrejoinChrome(win){
    ...
    if(!preview.querySelector('[data-ds-prejoin-backgrounds]')){
      const background=document.createElement('button');background.type='button';background.dataset.dsPrejoinBackgrounds='1';background.className='ds-prejoin-backgrounds';background.textContent='▣ Backgrounds';background.onclick=()=>{const dialog=q('#settingsDialog');if(dialog&&!dialog.open)dialog.showModal();};preview.append(background);
    }
    let preference=win.querySelector('.ds-prejoin-always');
    if(!preference){
      preference=document.createElement('label');preference.className='ds-prejoin-always';preference.innerHTML='<input type="checkbox"><span>Always show this preview when joining</span>';
      const footer=win.querySelector('footer');footer?.insertAdjacentElement('beforebegin',preference);
      const input=preference.querySelector('input');input.checked=window.DominionPreferences?.read?.('showJoinPreview')!==false;input.onchange=()=>window.DominionPreferences?.write?.('showJoinPreview',input.checked);
    }
  }

  function syncPrejoin(){
    const overlay=q('#prejoinOverlay'),win=overlay?.querySelector('.prejoin-window');if(!overlay||overlay.hidden||!win)return;
    overlay.classList.add('ds-adaptive-prejoin');win.classList.add('ds-adaptive-prejoin-window');ensurePrejoinChrome(win);
    const deviceLabels=qa('#prejoinOverlay .device-grid label');for(const label of deviceLabels){const title=String(label.querySelector('span')?.textContent||'').trim().toLowerCase();label.hidden=title==='speaker';}
    const mirror=q('#prejoinOverlay .mirror-option');if(mirror)mirror.hidden=true;
  }
  ...
  document.addEventListener('click',event=>{
    if(event.target.closest?.('#roomParticipants'))requestAnimationFrame(()=>{const side=q('.room-side');if(side&&!side.hidden){side.dataset.dsAdaptiveInitialized='';syncParticipants();}});
