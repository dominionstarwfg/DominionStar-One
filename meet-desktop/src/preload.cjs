...
contextBridge.exposeInMainWorld('dominionDesktop',Object.freeze({
  ...
  share:Object.freeze({
    ...
    captureStarted:state=>{ipcRenderer.send('share:capture-started',state||{});return true;},captureState:state=>invoke('share:capture-state',state),presenterCommitted:state=>{const snapshot=state||{};ipcRenderer.send('share:presenter-committed',snapshot);if(String(snapshot.sourceName||'')==='QA Synthetic Share'){setTimeout(()=>console.log('QA_PRESENTER_PRELOAD_HEARTBEAT post-commit'),700);}return true;},captureStopped:()=>invoke('share:capture-stopped'),onPresenterCommand:callback=>listenPresenterCommand(callback)
  }),
