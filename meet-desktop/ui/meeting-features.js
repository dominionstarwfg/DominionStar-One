(()=>{
  ...
  function openReactions(anchor=q('#roomReactions')){ensureUi();closeReactionMenu();if(!anchor)return;const menu=document.createElement('div');menu.className='meeting-reaction-menu';for(const emoji of reactions){const b=document.createElement('button');b.type='button';b.className='reaction-emoji-button';b.textContent=emoji;b.setAttribute('aria-label',`React ${emoji}`);b.style.setProperty('width','48px','important');b.style.setProperty('min-width','48px','important');b.style.setProperty('height','48px','important');b.style.setProperty('min-height','48px','important');b.style.setProperty('font-size','25px','important');b.style.setProperty('line-height','1','important');b.style.setProperty('padding','0','important');b.onclick=()=>{closeReactionMenu();void sendReaction(emoji);};menu.append(b);}document.body.append(menu);state.reactionMenu=menu;const r=anchor.getBoundingClientRect(),width=menu.offsetWidth,height=menu.offsetHeight;const left=Math.max(10,Math.min(innerWidth-width-10,r.left+r.width/2-width/2));const top=Math.max(10,r.top-height-10);menu.style.left=`${left}px`;menu.style.right='auto';menu.style.top=`${top}px`;menu.style.bottom='auto';}
