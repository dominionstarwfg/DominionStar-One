(()=>{
  ...
  const prefValue=(key,fallback='')=>{try{const value=localStorage.getItem(key);return value===null?fallback:value;}catch{return fallback;}};
  const saveValue=(key,value)=>{try{localStorage.setItem(key,String(value??''));}catch{}};
  let root=null;
  let recovery=null;
  let sources=[];
  let selectedId='';
  ...
  let presenterWallpaper='';
  const previewGeometryByLayout=new Map();
  const clamp=(value,min,max)=>Math.max(min,Math.min(max,value));
  const cloneRect=rect=>({x:Number(rect.x),y:Number(rect.y),w:Number(rect.w),h:Number(rect.h)});
  const defaultGeometry=layout=>{
  ...
  const geometryFor=layout=>{const key=String(layout||'content');if(!previewGeometryByLayout.has(key))previewGeometryByLayout.set(key,defaultGeometry(key));return previewGeometryByLayout.get(key);};
  const stopPreviewCamera=()=>{for(const track of previewCameraStream?.getTracks?.()||[]){try{track.stop();}catch{}}previewCameraStream=null;};
  ...
  function ensureStyle(){
    ...
    style.textContent=`
      ...
      .ds2041-share-right{min-width:0;min-height:0;padding:12px 14px;background:#292a2c;overflow:auto}.ds2041-presenter-head{height:24px;display:flex;align-items:center;justify-content:space-between;font-size:10px}.ds2041-preview-expand{width:24px;height:22px;border:0;border-radius:5px;background:transparent;color:#cfd1d5;cursor:pointer}.ds2041-preview{position:relative;height:118px;border:1px solid #404145;border-radius:4px;background:#17181a;display:grid;place-items:center;overflow:hidden;color:#7f8083;font-size:9px;touch-action:none}.ds2041-preview>img{width:100%;height:100%;object-fit:contain}.ds2041-preview-stage{position:absolute;inset:0;background:#080a0d;overflow:hidden}.ds2041-preview-item{position:absolute;border:1px solid rgba(255,255,255,.5);box-sizing:border-box;overflow:hidden;cursor:grab;touch-action:none}.ds2041-preview-item.dragging{cursor:grabbing;border-color:#1688ff;box-shadow:0 0 0 1px #1688ff}.ds2041-preview-item.content{background:#111}.ds2041-preview-item.content img{width:100%;height:100%;object-fit:contain}.ds2041-preview-item.camera{background:#111;display:grid;place-items:center;color:#b9bbc0}.ds2041-preview-item.camera video{width:100%;height:100%;object-fit:cover;transform:scaleX(-1)}.ds2041-preview-handle{position:absolute;right:-1px;bottom:-1px;width:11px;height:11px;background:#1688ff;border:1px solid #fff;border-radius:2px;cursor:nwse-resize}.ds2041-share-window.preview-expanded{width:min(1120px,calc(100vw - 28px));height:min(760px,calc(100vh - 28px))}.ds2041-share-window.preview-expanded .ds2041-share-main{grid-template-columns:minmax(0,1fr) 500px}.ds2041-share-window.preview-expanded .ds2041-preview{height:320px}.ds2041-layout-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:8px}.ds2041-layout{height:58px;padding:5px;border:1px solid #3b3c40;border-radius:7px;background:#2a2b2d;color:#ddd;font-size:8.5px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;cursor:pointer}.ds2041-layout.active{border:2px solid #1688ff;background:#30353a}.ds2041-layout:disabled{opacity:.42;cursor:default}.ds2041-layout-glyph{width:48px;height:22px;border:1.4px solid #ececee;border-radius:3px;position:relative}.ds2041-layout.side .ds2041-layout-glyph:after{content:"";position:absolute;top:0;bottom:0;left:50%;border-left:1.3px solid #fff}.ds2041-wallpaper{display:flex;gap:6px;margin-top:8px}.ds2041-wallpaper[hidden]{display:none}.ds2041-wallpaper button{height:26px;border:1px solid #45474c;border-radius:6px;background:#25272a;color:#eceef2;font-size:8.5px;padding:0 9px;cursor:pointer}.ds2041-wallpaper button:disabled{opacity:.45;cursor:default}.ds2041-share-options{margin-top:14px;padding-top:8px}.ds2041-options-title{margin-bottom:7px;font-size:10px;font-weight:600}.ds2041-option{min-height:30px;display:grid;grid-template-columns:16px minmax(0,1fr);align-items:center;gap:6px;color:#eee;font-size:9.5px}.ds2041-option input{width:14px;height:14px;margin:0;accent-color:#0e72ed}.ds2041-option.sound{grid-template-columns:16px minmax(0,1fr) auto}.ds2041-audio-mode{height:26px;border:1px solid #45474c;border-radius:6px;background:#202226;color:#f1f1f2;font-size:8.5px;padding:0 6px}.ds2041-more{display:grid;gap:10px}.ds2041-more-card{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:14px;align-items:center;padding:14px;border:1px solid #404145;border-radius:7px;background:#292a2d}.ds2041-more-card strong{font-size:10px}.ds2041-more-card p{margin:4px 0 0;color:#8f9094;font-size:9px}.ds2041-more-card input{accent-color:#0e72ed}
  ...
  function ensureRoot(){
    ...
    root.innerHTML=`<div class="ds2041-share-window"><header class="ds2041-share-head"><span class="ds2041-share-dot" aria-hidden="true"></span><span class="ds2041-share-brand"><img alt=""></span><nav class="ds2041-share-tabs"><button type="button" data-tab="screens" class="active">Screens</button><button type="button" disabled title="File presenting is not certified yet">Files</button><button type="button" data-tab="more">More</button></nav><button type="button" class="ds2041-share-close" aria-label="Close">×</button></header><section class="ds2041-share-main"><div class="ds2041-share-browser"><span class="ds2041-share-status"></span><div class="ds2041-share-content"></div></div><aside class="ds2041-share-right"><section class="ds2041-presenter"><div class="ds2041-presenter-head"><strong>Presenter layout</strong><button type="button" class="ds2041-preview-expand" aria-label="Expand presenter preview">↗</button></div><div class="ds2041-preview"><span>Select a screen or window</span></div><div class="ds2041-layout-grid"><button class="ds2041-layout active" type="button" data-layout="content"><span class="ds2041-layout-glyph"></span><span>Content only</span></button><button class="ds2041-layout background" type="button" data-layout="background"><span class="ds2041-layout-glyph"></span><span>As background</span></button><button class="ds2041-layout shoulder" type="button" data-layout="shoulder"><span class="ds2041-layout-glyph"></span><span>Over the shoulder</span></button><button class="ds2041-layout side" type="button" data-layout="side" title="Combines your camera and shared content into one participant-facing frame"><span class="ds2041-layout-glyph"></span><span>Side by side</span></button></div><div class="ds2041-wallpaper" data-presenter-wallpaper-controls hidden><button type="button" data-presenter-wallpaper-choose>Choose wallpaper</button><button type="button" data-presenter-wallpaper-remove disabled>Remove wallpaper</button><input type="file" accept="image/png,image/jpeg,image/webp" data-presenter-wallpaper-file hidden></div></section><section class="ds2041-share-options"><div class="ds2041-options-title">Share options</div><label class="ds2041-option sound"><input type="checkbox" data-share-audio><span>Share sound</span><select class="ds2041-audio-mode" data-share-audio-mode aria-label="Share sound quality"><option value="mono">Mono</option><option value="stereo">Stereo (high fidelity)</option></select></label><label class="ds2041-option"><input type="checkbox" data-optimize><span>Optimize for video sharing</span></label><label class="ds2041-option"><input type="checkbox" data-include-meet><span>Share DominionStar Meet windows</span></label></section></aside></section><footer class="ds2041-share-footer"><button type="button" class="ds2041-share-button" disabled>Share</button></footer></div>`;
    ...
    const audioMode=root.querySelector('[data-share-audio-mode]');if(audioMode)audioMode.value=prefValue('ds_pref_share_audio_mode','mono')==='stereo'?'stereo':'mono';
    ...
    root.querySelector('.ds2041-preview-expand').onclick=()=>{previewExpanded=!previewExpanded;root.querySelector('.ds2041-share-window')?.classList.toggle('preview-expanded',previewExpanded);root.querySelector('.ds2041-preview-expand').textContent=previewExpanded?'↙':'↗';syncSelected();};
    root.querySelector('[data-presenter-wallpaper-choose]').onclick=()=>root.querySelector('[data-presenter-wallpaper-file]')?.click();
    root.querySelector('[data-presenter-wallpaper-remove]').onclick=()=>{presenterWallpaper='';syncWallpaperControls();syncSelected();};
    root.querySelector('[data-presenter-wallpaper-file]').onchange=event=>void loadPresenterWallpaper(event.currentTarget.files?.[0]);
    root.querySelectorAll('[data-layout]').forEach(button=>button.onclick=()=>{if(button.disabled)return;root.querySelectorAll('[data-layout]').forEach(item=>item.classList.toggle('active',item===button));const layout=String(button.dataset.layout||'content');geometryFor(layout);if(layout!=='content')void ensurePreviewCamera();syncWallpaperControls();syncSelected();});
    ...
    root.querySelector('[data-share-audio-mode]').onchange=event=>saveValue('ds_pref_share_audio_mode',event.currentTarget.value==='stereo'?'stereo':'mono');
  ...
  function syncWallpaperControls(){
    const layout=activePresenterLayout(),controls=root?.querySelector('[data-presenter-wallpaper-controls]'),remove=root?.querySelector('[data-presenter-wallpaper-remove]');
    if(controls)controls.hidden=!(layout==='shoulder'||layout==='side');
    if(remove)remove.disabled=!presenterWallpaper;
  }
  async function loadPresenterWallpaper(file){
    if(!file||!/^image\/(png|jpeg|webp)$/i.test(String(file.type||'')))return false;
    try{
      const bitmap=await createImageBitmap(file),maxEdge=1600,scale=Math.min(1,maxEdge/Math.max(bitmap.width,bitmap.height)),canvas=document.createElement('canvas');
      ...
      presenterWallpaper=canvas.toDataURL('image/jpeg',.86);syncWallpaperControls();syncSelected();return true;
    }catch{return false;}
  }

  async function ensurePreviewCamera(){
    if(previewCameraStream?.getVideoTracks?.().some(track=>track.readyState==='live'))return previewCameraStream;
    try{previewCameraStream=await window.DominionMediaController?.testCameraStream?.();syncSelected();return previewCameraStream;}catch{return null;}
  }
  const activePresenterLayout=()=>String(root?.querySelector('[data-layout].active')?.dataset.layout||'content');
  function applyPreviewRects(preview,layout){
    const geometry=geometryFor(layout);
    for(const role of ['camera','content']){
      const node=preview.querySelector(`[data-preview-role="${role}"]`),rect=geometry[role];if(!node||!rect)continue;
      node.style.left=`${rect.x*100}%`;node.style.top=`${rect.y*100}%`;node.style.width=`${rect.w*100}%`;node.style.height=`${rect.h*100}%`;
    }
  }
  function bindPreviewItem(item,layout){
    item.onpointerdown=event=>{
      if(event.button!==0)return;event.preventDefault();event.stopPropagation();const preview=item.closest('.ds2041-preview'),role=String(item.dataset.previewRole||''),geometry=geometryFor(layout),rect=cloneRect(geometry[role]),box=preview.getBoundingClientRect(),resizing=Boolean(event.target.closest('.ds2041-preview-handle')),startX=event.clientX,startY=event.clientY;item.classList.add('dragging');item.setPointerCapture?.(event.pointerId);
      item.onpointermove=move=>{if(!item.hasPointerCapture?.(event.pointerId))return;const dx=(move.clientX-startX)/Math.max(1,box.width),dy=(move.clientY-startY)/Math.max(1,box.height),minW=role==='camera'?.16:.24,minH=role==='camera'?.16:.20;
        if(resizing){rect.w=clamp(rect.w+dx,minW,1-rect.x);rect.h=clamp(rect.h+dy,minH,1-rect.y);}
        else{rect.x=clamp(rect.x+dx,0,1-rect.w);rect.y=clamp(rect.y+dy,0,1-rect.h);}
        geometry[role]=cloneRect(rect);applyPreviewRects(preview,layout);
      };
      item.onpointerup=()=>{item.classList.remove('dragging');try{item.releasePointerCapture?.(event.pointerId);}catch{}item.onpointermove=null;item.onpointerup=null;};
    };
  }
  function syncSelected(){
    ...
    const layout=activePresenterLayout();
    if(!source){preview.innerHTML='<span>Select a screen or window</span>';return;}
    if(layout==='content'){preview.innerHTML=source.thumbnail?`<img src="${esc(source.thumbnail)}" alt="">`:`<span>${esc(displayName(source))}</span>`;return;}
    const cameraReady=previewCameraStream?.getVideoTracks?.().some(track=>track.readyState==='live');
    preview.innerHTML=`<div class="ds2041-preview-stage"><div class="ds2041-preview-item content" data-preview-role="content">${source.thumbnail?`<img src="${esc(source.thumbnail)}" alt="">`:`<span>${esc(displayName(source))}</span>`}<span class="ds2041-preview-handle"></span></div><div class="ds2041-preview-item camera" data-preview-role="camera">${cameraReady?'<video autoplay muted playsinline></video>':'<span>Camera preview</span>'}<span class="ds2041-preview-handle"></span></div></div>`;
    const stage=preview.querySelector('.ds2041-preview-stage');if(stage&&presenterWallpaper&&(layout==='shoulder'||layout==='side')){stage.style.backgroundImage=`url("${presenterWallpaper}")`;stage.style.backgroundSize='cover';stage.style.backgroundPosition='center';}
    if(layout==='background'){const content=preview.querySelector('[data-preview-role="content"]');if(content){content.style.pointerEvents='none';content.querySelector('.ds2041-preview-handle')?.remove();}}
    const video=preview.querySelector('[data-preview-role="camera"] video');if(video&&previewCameraStream){video.srcObject=previewCameraStream;void video.play().catch(()=>{});}
    applyPreviewRects(preview,layout);preview.querySelectorAll('.ds2041-preview-item').forEach(item=>{if(item.style.pointerEvents!=='none')bindPreviewItem(item,layout);});
  }
  ...
  async function open(){
    if(!pickerBridge?.listSources||!pickerBridge?.choose)return false;presenterWallpaper='';
  ...
  function close(){stopAutoRefresh();stopPreviewCamera();previewExpanded=false;if(root){root.hidden=true;root.querySelector('.ds2041-share-window')?.classList.remove('preview-expanded');}}
  ...
  async function commit(){
    ...
    const presenterLayout=String(root.querySelector('[data-layout].active')?.dataset.layout||'content');
    if(presenterLayout!=='content'){
    ...
    const geometry=geometryFor(presenterLayout);
    const presenterGeometry={camera:cloneRect(geometry.camera),content:cloneRect(geometry.content)};
    const options={
      presenterLayout,
      presenterGeometry,
      presenterWallpaper:(presenterLayout==='shoulder'||presenterLayout==='side')?presenterWallpaper:'',
    ...
    try{
      ...
      save('ds_pref_share_audio',options.shareAudio);saveValue('ds_pref_share_audio_mode',options.shareAudioMode);save('ds_pref_share_optimize',options.optimizeVideo);save('ds_pref_share_include_meet',options.includeMeetWindows);
