...
if(String(pkg.version)!=='2.0.42')throw new Error(`2.0.42 physical-reference gate requires DominionStar Meet 2.0.42; found ${pkg.version}`);
...
console.log('DOMINIONSTAR_PHYSICAL_PARITY_2_0_42_OK custom-only-preshare screens-files-more presenter-layout no-apple-overlay real-brand view-modes adaptive-participants participant-native-mouse-drag two-person-right-filmstrip video-filmstrip-native-mouse-drag narrow-only-top-reflow compact-prejoin physical-rejection-recorded');
