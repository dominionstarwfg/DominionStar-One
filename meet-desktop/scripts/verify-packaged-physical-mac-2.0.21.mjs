...
try{
  ...
  await waitFor("document.querySelector('.ds-reaction-tray')",'live physical reaction tray');
  await sleep(40);
  const reaction=await evaluate(`(()=>{const tray=document.querySelector('.ds-reaction-tray'),visible=[...tray.querySelectorAll('button:not(.ds-raise-hand)')].filter(b=>getComputedStyle(b).display!=='none'),hand=tray.querySelector('.ds-raise-hand'),divider=tray.querySelector('.ds-reaction-divider'),dedicated=document.querySelector('#roomRaiseHand');return {reactions:visible.length,handVisible:Boolean(hand&&getComputedStyle(hand).display!=='none'),dividerVisible:Boolean(divider&&getComputedStyle(divider).display!=='none'),dedicatedVisible:Boolean(dedicated&&!dedicated.hidden&&getComputedStyle(dedicated).display!=='none'),overflow:getComputedStyle(tray).overflow,buttonWidth:visible[0]?.getBoundingClientRect().width||0,font:visible[0]?parseFloat(getComputedStyle(visible[0]).fontSize):0};})()`);
  assert.equal(reaction.reactions,6,'Physical reaction tray must retain six standard reactions.');
  assert.equal(reaction.handVisible,false,'Legacy reaction-tray Raise Hand must not be visible in 2.0.22.');
  assert.equal(reaction.dividerVisible,false,'Unused reaction-tray divider must not be visible with the legacy hand section.');
