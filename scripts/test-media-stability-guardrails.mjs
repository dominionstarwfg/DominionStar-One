...
assert(background.includes('/float16/1/selfie_segmenter_landscape.tflite')&&!background.includes('/float16/latest/'),'Background segmentation must use a pinned model asset, never an unversioned latest model.');
assert(background.includes("const audioTracks = rawStream.getAudioTracks().filter(track => track.readyState === 'live')"),'Background processing must preserve the live microphone tracks from the raw media stream.');
...
assert(desktopBootstrap.includes('screen-permission-lifecycle.mjs'),'The desktop bootstrap must load the native screen permission lifecycle controller.');
assert(!operationBootstrap.includes('screen-permission-ui-guard.js'),'The active desktop runtime must not load a second browser permission authority.');
assert(sharePicker.includes('state?.requiresRestart')&&sharePicker.includes('Restart DominionStar Meet')&&sharePicker.includes('getScreenPermissionStatus'),'The share picker must distinguish granted-but-stale sessions from denied permission and offer one controlled restart.');
assert(sharePicker.includes("'Screen access is active'")&&sharePicker.includes('SOURCE_RETRY_DELAYS')&&sharePicker.includes('settingsButton.hidden=granted'),'Granted permission must retry real source enumeration instead of looping back to Settings.');

assert(meetingIdentity.includes('ds-personal-room-heading')&&meetingIdentity.includes('Set your meeting identity and Personal Room defaults.'),'Meeting Settings must use a clear Personal Room section and concise hierarchy.');
assert(!meetingIdentity.includes('<input id="generatedWaitingRoom"')&&!meetingIdentity.includes('<input id="generatedRequirePasscode"'),'Meeting Settings must not duplicate Waiting Room/passcode controls for generated meetings inside Personal Room settings.');

for(const legacy of ['🎙','◉','♙','▢','Ⅱ','↗']) assert(!presenterToolbar.includes(legacy),`The native presenter toolbar must not use legacy glyph ${legacy}.`);
assert(presenterToolbar.includes('<svg')&&presenterToolbar.includes('data-command="new-share"')&&presenterToolbar.includes('data-command="pause"')&&presenterToolbar.includes('data-command="stop"'),'The native presenter toolbar must use vector controls for the core Zoom-style sharing actions.');
assert(ui.includes('window.dominionDesktop.updatePresenterDock?.({tiles})'),'The meeting runtime must publish live participant tiles while desktop sharing is active.');
assert(desktopPreload.includes("updatePresenterDock: state => ipcRenderer.send('desktop:presenter-dock-update'"),'The desktop preload must expose the native participant dock bridge.');
assert(desktopBootstrap.includes('presenter-dock.mjs'),'The desktop bootstrap must load the native presenter dock controller.');
assert(presenterDockMain.includes('alwaysOnTop:true')&&presenterDockMain.includes('resizable:true')&&presenterDockMain.includes('desktop:presenter-dock-update'),'The native participant dock must be a resizable always-on-top sharing surface.');
assert(presenterDockHtml.includes('-webkit-app-region:drag')&&presenterDockHtml.includes('Participant video will appear here while you share.'),'The native participant dock must remain movable and provide a clean empty state.');

assert(engine.includes('context.drawImage(video,0,0,width,height)')&&engine.includes('const freezeStream=canvas.captureStream(1)'),'Pause Share must freeze the last rendered frame rather than blank the receiver.');
assert(engine.includes('const frozen=await createFrozenScreenTrack()')&&engine.includes('state.screenPaused=true')&&engine.includes('syncPeerTracks(peer)'),'Pause Share must replace only the outgoing presentation sender with the frozen frame.');
assert(engine.includes('state.screenPaused=false')&&engine.includes('clearFrozenScreenTrack()'),'Resume Share must return receivers to the live capture and dispose the temporary frozen track.');

assert(shareLifecycle.includes('app.dock.isVisible()')&&shareLifecycle.includes('await app.dock.show()'),'Screen sharing must actively preserve the DominionStar macOS Dock icon.');
assert(shareLifecycle.includes('desktop:presenter-show')&&shareLifecycle.includes('desktop:presenter-hide'),'Dock visibility must be guarded for the full presenter lifecycle.');
assert(desktopBootstrap.includes('share-lifecycle.mjs'),'The desktop bootstrap must load the share-lifecycle guard.');

assert(presenterToolbarJs.includes("document.body.classList.add('stopping','collapsed')")&&presenterToolbarJs.includes("document.body.style.opacity='0'"),'Stop Share must immediately retire the native presenter controls.');
assert(presenterToolbarJs.includes("window.presenterBridge.command('stop')")&&presenterToolbarJs.includes('stopRecoveryTimer=setTimeout'),'Stop Share must still execute the real engine command and recover controls if teardown fails.');

assert(desktopPreload.includes('showRemoteControlPrompt')&&desktopPreload.includes('onRemoteControlDecision')&&desktopPreload.includes('showRemoteControlError'),'The preload bridge must expose the complete remote-control approval lifecycle.');
assert(remoteControlDialog.includes('desktop:remote-control-prompt')&&remoteControlDialog.includes("buttons: ['Deny', 'Approve']"),'Desktop remote control must present an explicit trusted-user approval dialog.');
assert(remoteControlDialog.includes('desktop:remote-control-error')&&remoteControlDialog.includes('dialog.showMessageBox'),'Remote-control permission failures must be surfaced through a native desktop dialog.');
assert(desktopBootstrap.includes('remote-control-dialog.mjs'),'The desktop bootstrap must load the native remote-control approval UI.');
