const normalizeRoomCode=value=>String(value||'').replace(/\D/g,'').slice(0,11);
const normalizePasscode=value=>String(value||'').replace(/\D/g,'').slice(0,7);
...
export function createMeetingService({auth,allowDirectQa=false}){
  ...
  const remember=value=>{
    if(value?.roomId)current={
      roomId:String(value.roomId),roomCode:String(value.roomCode||current.roomCode||''),passcode:String(value.passcode||current.passcode||''),title:String(value.title||current.title||''),
      participantId:String(value.participantId||current.participantId||''),joinToken:String(value.joinToken||current.joinToken||''),role:String(value.role||current.role||''),state:String(value.state||current.state||''),
      meetingKind:String(value.meetingKind||current.meetingKind||''),reusable:Boolean(value.reusable??current.reusable),scheduleId:String(value.scheduleId||current.scheduleId||'')
    };
    return value;
  };
  const clear=()=>{current={roomId:'',roomCode:'',passcode:'',title:'',participantId:'',joinToken:'',role:'',state:'',meetingKind:'',reusable:false,scheduleId:''};turnCache={roomId:'',iceServers:[],expiresAtMs:0,provider:'',ttl:0,qaDirectOnly:false};};
  const assertPasscode=passcode=>{if(!validPasscode(passcode))throw new Error('Passcode must contain 3 to 7 digits.');};

  async function createRoom(input={}){
    const passcode=normalizePasscode(input.passcode);assertPasscode(passcode);const title=normalizeTitle(input.title);
  ...
  async function updatePersonalRoom(input={}){
    const passcode=normalizePasscode(input.passcode);assertPasscode(passcode);
    const result=await auth.rpc('meet_v2_update_personal_room',{p_passcode:passcode,p_use_for_instant:input.useForInstant!==false,p_waiting_room_enabled:input.waitingRoomEnabled!==false,p_external_guests_allowed:input.externalGuestsAllowed!==false});
  ...
  async function startPersonalRoom(){return remember({...await auth.rpc('meet_v2_start_personal_room',{}),meetingKind:'personal',reusable:true});}
  async function startHostRoom(roomId){return remember(await auth.rpc('meet_v2_start_host_room',{p_room_id:roomId}));}
  async function scheduleRoom(input={}){
    const title=normalizeTitle(input.title),usePersonalRoom=Boolean(input.usePersonalRoom),passcode=usePersonalRoom?'':normalizePasscode(input.passcode);
    if(!usePersonalRoom)assertPasscode(passcode);
    const startsAt=new Date(input.scheduledStart||input.startsAt||'');if(!Number.isFinite(startsAt.getTime()))throw new Error('Choose a valid meeting date and time.');
    const duration=Math.max(15,Math.min(480,Number(input.durationMinutes)||60));const recurrence=input.recurrence&&typeof input.recurrence==='object'?input.recurrence:null;
    const result=await auth.rpc('meet_v2_schedule_meeting',{p_title:title,p_passcode:passcode||null,p_scheduled_start:startsAt.toISOString(),p_duration_minutes:duration,p_recurrence:recurrence,p_waiting_room_enabled:input.waitingRoomEnabled!==false,p_external_guests_allowed:input.externalGuestsAllowed!==false,p_use_personal_room:usePersonalRoom});
  ...
  async function updateRoomPasscode(roomId,value){const passcode=normalizePasscode(value);assertPasscode(passcode);const result=await auth.rpc('meet_v2_update_room_passcode',{p_room_id:roomId,p_passcode:passcode});if(current.roomId===String(roomId))current.passcode=passcode;return result;}
  async function requestJoin(input={}){
    const roomCode=normalizeRoomCode(input.roomCode),passcode=normalizePasscode(input.passcode),displayName=normalizeName(input.displayName);
    if(!validRoomCode(roomCode))throw new Error('Meeting ID must contain 10 or 11 digits.');assertPasscode(passcode);if(!displayName)throw new Error('Enter your display name.');
  ...
  async function iceConfig({force=false,ttl=7200}={}){
    if(!current.roomId||current.state!=='joined')throw new Error('meeting_context_missing');const now=Date.now(),cachedReady=hasRelay(turnCache.iceServers)||turnCache.qaDirectOnly===true;
    if(!force&&turnCache.roomId===current.roomId&&turnCache.expiresAtMs-now>10*60*1000&&cachedReady)return cloneIce(turnCache);
    try{const data=await auth.invokeServerFunction('meet-v2-turn-credentials',{roomId:current.roomId,ttl:Math.max(900,Math.min(Number(ttl)||7200,43200))});const servers=Array.isArray(data?.iceServers)?data.iceServers.filter(server=>server&&server.urls):[];if(!hasRelay(servers))throw new Error('turn_relay_unavailable');const expiresAtMs=Date.parse(String(data?.expiresAt||''));if(!Number.isFinite(expiresAtMs)||expiresAtMs<=now+5*60*1000)throw new Error('turn_credentials_expire_too_soon');turnCache={roomId:current.roomId,iceServers:servers,expiresAtMs,provider:String(data?.provider||'relay'),ttl:Number(data?.ttl)||0,qaDirectOnly:false};return cloneIce(turnCache);}catch(error){if(!allowDirectQa)throw error;return directQaConfig(now);}
  }
  return Object.freeze({createRoom,personalRoom,updatePersonalRoom,startPersonalRoom,startHostRoom,scheduleRoom,listSchedules,cancelSchedule,startSchedule,updateRoomPasscode,requestJoin,joinStatus,markJoined,leaveRoom,hostQueue,decide,snapshot,setCohost,removeParticipant,endRoom,context,sendSignal,pullSignals,pruneSignals,iceConfig});
}
