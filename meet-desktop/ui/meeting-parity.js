(()=>{
  ...
  function syncActiveSpeakerStage(){const activeStage=ensureActiveSpeakerStage();if(!activeStage)return false;qa('.remote-peer-tile.stage-promoted').forEach(tile=>tile.classList.remove('stage-promoted'));if(sharing()){activeStage.hidden=true;activeStage.srcObject=null;q('#meetingOverlay')?.classList.remove('remote-speaker-stage');return false;}const tile=q('#remoteTileStrip .remote-peer-tile.active-speaker'),source=tile?.querySelector('video');if(!source?.srcObject||source.hidden){activeStage.hidden=true;activeStage.srcObject=null;q('#meetingOverlay')?.classList.remove('remote-speaker-stage');return false;}activeStage.srcObject=source.srcObject;activeStage.hidden=false;tile.classList.add('stage-promoted');q('#meetingOverlay')?.classList.add('remote-speaker-stage');void activeStage.play().catch(()=>{});return true;}
  function syncLocalDockTile(remotePromoted=false){const dock=ensureVideoDock(),tile=q('#localVideoDockTile');if(!dock||!tile)return;const snapshot=media()?.snapshot?.()||{},stream=media()?.stream?.()||null,should=Boolean(snapshot.videoLive&&(sharing()||remotePromoted));tile.hidden=!should;const video=tile.querySelector('video');if(should){if(video.srcObject!==stream)video.srcObject=stream;tile.querySelector('.remote-peer-fallback').hidden=true;void video.play().catch(()=>{});}else{video.srcObject=null;tile.querySelector('.remote-peer-fallback').hidden=false;}}
  function syncVideoDock(){const dock=ensureVideoDock();if(!dock)return;const promoted=syncActiveSpeakerStage();syncLocalDockTile(promoted);const tiles=qa('#participantVideoDock .remote-peer-tile').filter(tile=>!tile.hidden&&!tile.classList.contains('stage-promoted')),count=tiles.length;dock.dataset.count=String(Math.min(count,9));dock.classList.toggle('dock-empty',count===0);dock.hidden=count===0;for(let i=1;i<=9;i++)dock.classList.toggle(`count-${i}`,Math.min(count,9)===i);const anchor=dock.dataset.anchor||'right';dock.dataset.orientation=(anchor==='top'||anchor==='bottom')?'horizontal':'vertical';if(!dock.classList.contains('user-positioned')){dock.style.left='';dock.style.top='';dock.style.right=anchor==='left'?'auto':'14px';dock.style.bottom=anchor==='bottom'?'14px':'auto';if(anchor==='left')dock.style.left='14px';else if(anchor==='top')dock.style.top='14px';else if(anchor==='right')dock.style.top='14px';}}
  ...
  const observer=new MutationObserver(()=>{install();syncVideoDock();decorateControls();});observer.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['hidden','class']});
  ...
  window.addEventListener('resize',()=>{closeMenus();restorePanelGeometry();restoreVideoDock();syncVideoDock();},{passive:true});
  setInterval(()=>{syncGreeting();if(meetingOpen()){void syncMeetingMeta();syncVideoDock();}},700);
  install();
  window.DominionMeetingParity=Object.freeze({version:'2.0.0-zoom-adaptive-dock',install,toggleParticipants,syncVideoDock,resetVideoDock,syncMeetingMeta});
})();
