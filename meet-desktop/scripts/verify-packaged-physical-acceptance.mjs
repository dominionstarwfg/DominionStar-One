...
try{
  ...
  const hostMenu=await evaluate(`(()=>{const m=document.querySelector('.ds-ref-host-tools-panel'),r=m.getBoundingClientRect(),style=getComputedStyle(m),heading=m.querySelector('header strong')?.textContent||'',labels=[...m.querySelectorAll('label span:first-child')].map(n=>n.textContent.trim()),buttons=[...m.querySelectorAll('button')].map(b=>b.textContent.trim());return {z:parseInt(style.zIndex)||0,position:style.position,width:Math.round(r.width),labelFont:Math.min(...[...m.querySelectorAll('label,.ds-ref-host-nav')].map(n=>parseFloat(getComputedStyle(n).fontSize)||99)),bottomGap:Math.round(innerHeight-r.bottom),rightGap:Math.round(innerWidth-r.right),computedBottom:parseFloat(style.bottom)||0,heading,labels,buttons,hasLock:Boolean(m.querySelector('[data-lock]')),hasWaiting:Boolean(m.querySelector('[data-waiting]')),hasParticipants:Boolean(m.querySelector('[data-participants]')),hasAdvanced:Boolean(m.querySelector('[data-advanced]'))};})()`);
  ...
  assert.ok(hostMenu.width>=340&&hostMenu.labelFont>=12.5,'Host Tools must remain readable rather than collapsing to the retired tiny 2.0.41 surface.');
  ...
  await evaluate(`(()=>{document.querySelector('#roomParticipants').click();const roster=document.querySelector('#participantRoster');roster.innerHTML='<div class="person-row" data-participant-id="qa-self" data-participant-self="1" data-participant-role="host" data-participant-name="QA Host"><span class="person-badge">QH</span><span class="person-copy"><strong>QA Host (Host) (me)</strong><small>Host</small></span><span class="participant-actions"><button class="ds-host-row-more">•••</button><button data-ds-self-more>•••</button></span></div><div class="person-row" data-participant-id="qa-guest" data-participant-role="participant" data-participant-name="Taylor Participant" data-recording-allowed="0" data-record-eligible="1"><span class="person-badge">TP</span><span class="person-copy"><strong>Taylor Participant</strong><small>Participant</small></span></div>';window.DominionZoomPhysicalAcceptance.decorateParticipantRows();window.DominionRuntimeStability.syncParticipantsSurface();window.DominionRuntimeStability.layoutSideSurface();return true;})()`);
  ...
  assert.equal(participantRow.traffic,3,'Participants must expose Mac-style close, minimize, and zoom controls.');
  ...
  assert.equal(chat.traffic,3,'Chat must expose Mac-style close, minimize, and zoom controls.');
  ...
  const reactionTray=await evaluate(`(()=>{const tray=document.querySelector('.meeting-reaction-menu'),buttons=[...tray.querySelectorAll('.reaction-emoji-button')],dedicated=document.querySelector('#roomRaiseHand'),anchor=document.querySelector('#roomReactions'),tr=tray.getBoundingClientRect(),ar=anchor.getBoundingClientRect();return {z:parseInt(getComputedStyle(tray).zIndex)||0,reactions:buttons.filter(b=>getComputedStyle(b).display!=='none').length,legacyTrayAbsent:!document.querySelector('.ds-reaction-tray'),legacyHandSuppressed:!tray.querySelector('.reaction-hand-button')||getComputedStyle(tray.querySelector('.reaction-hand-button')).display==='none',dedicatedHand:Boolean(dedicated&&!dedicated.hidden&&getComputedStyle(dedicated).display!=='none'),pointer:getComputedStyle(tray).pointerEvents,minWidth:buttons.length?Math.min(...buttons.map(b=>b.getBoundingClientRect().width)):0,minFont:buttons.length?Math.min(...buttons.map(b=>parseFloat(getComputedStyle(b).fontSize)||0)):0,above:tr.bottom<=ar.top+2,centerDelta:Math.round(Math.abs((tr.left+tr.width/2)-(ar.left+ar.width/2)))};})()`);
  ...
  assert.equal(reactionTray.above,true,'Reaction tray must open above the React control.');
  assert.ok(reactionTray.centerDelta<=18,'Reaction tray must stay horizontally anchored to the React control.');
  ...
  console.log('DOMINIONSTAR_PACKAGED_PHYSICAL_ACCEPTANCE_OK view-click canonical-host-tools-panel more-click participant-media right-stable-draggable-participants right-stable-chat mac-panel-controls full-width-stage reaction-six-only reaction-above-control dedicated-raise-hand ten-second-left-reactions settings-readable runtime-owned-native-share stable-toolbar no-renderer-errors');
