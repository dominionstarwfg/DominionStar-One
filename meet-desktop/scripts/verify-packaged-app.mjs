...
assert(captions.includes("version:'1.2.0'")&&captions.includes("button.id='roomCaptions'")&&captions.includes("caret.id='roomCaptionsMenu'"),'Packaged toolbar must expose Show/Hide Captions and caption options.');
...
assert(captions.includes('Select manual captioner')&&captions.includes('meeting.setCaptionState')&&captions.includes('meeting.publishCaption')&&captions.includes("const canManage=()=>state.role==='host'"),'Packaged manual captioning must use host-only server-backed assignment and publication authority.');
