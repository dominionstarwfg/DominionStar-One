...
try{
  ...
  const prepared=await main.eval(`(async()=>{
    ...
    const makeStream=(kind='video')=>{const tracks=[makeTrack(kind)];return {getVideoTracks:()=>tracks.filter(track=>track.kind==='video'),getAudioTracks:()=>tracks.filter(track=>track.kind==='audio'),getTracks:()=>[...tracks],addTrack(track){if(track&&!tracks.includes(track))tracks.push(track);},removeTrack(track){const index=tracks.indexOf(track);if(index>=0)tracks.splice(index,1);}};};
