...
assert(runtime.includes("window.DominionZoomPhysicalAcceptance?.openHostTools?.(hostTools)")&&!runtime.includes("DominionMeetingParity?.openSecurity?.(hostTools)"),'Host Tools must have one canonical runtime owner.');
...
assert(physical.includes('openSmartSharePicker,openHostTools,decorateParticipantRows'),'Canonical Host Tools must remain callable after background reconciliation is retired.');
