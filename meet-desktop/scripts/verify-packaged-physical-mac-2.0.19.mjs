...
async function target(){const deadline=Date.now()+15000;while(Date.now()<deadline){if(child.exitCode!==null)throw new Error(`Packaged app exited before physical Mac gate.\n${stderr}`);try{const response=await fetch(`http://127.0.0.1:${port}/json/list`,{signal:AbortSignal.timeout(700)});if(response.ok){const targets=await response.json();const page=targets.find(item=>item.type==='page'&&String(item.url||'').startsWith('file://'));if(page?.webSocketDebuggerUrl)return page;}}catch{}await sleep(160);}throw new Error('Unable to attach to packaged renderer for physical Mac gate.');}
function connect(url){return new Promise((resolve,reject)=>{const socket=new WebSocket(url);const timer=setTimeout(()=>reject(new Error('Physical Mac CDP connection timeout.')),3000);socket.addEventListener('open',()=>{clearTimeout(timer);resolve(socket);},{once:true});socket.addEventListener('error',()=>{clearTimeout(timer);reject(new Error('Physical Mac CDP connection failed.'));},{once:true});});}
...
try{
  ...
  await waitFor("document.readyState==='complete'&&window.DominionPhysicalMacRepair",'physical Mac controller');
  ...
  assert.equal(controller.version,'2.0.20');assert.equal(controller.relaunch,true);assert.equal(controller.reset,true);assert.equal(controller.privacy,true);
  ...
  assert.equal(participant.text,'Participants (2)','Participants must expose live count in the heading owned by the actual roster panel.');
