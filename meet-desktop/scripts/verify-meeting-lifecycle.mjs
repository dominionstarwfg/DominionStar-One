...
for(const rpc of ['meet_v2_create_room','meet_v2_get_personal_room','meet_v2_update_personal_room','meet_v2_start_personal_room','meet_v2_schedule_meeting','meet_v2_list_host_schedules','meet_v2_mark_schedule_started','meet_v2_cancel_schedule','meet_v2_request_join','meet_v2_join_status','meet_v2_mark_joined','meet_v2_leave_room','meet_v2_host_queue','meet_v2_decide_participant','meet_v2_room_snapshot','meet_v2_set_cohost','meet_v2_remove_participant','meet_v2_rename_participant','meet_v2_set_security','meet_v2_set_chat_policy','meet_v2_set_caption_state','meet_v2_publish_caption','meet_v2_get_transcript','meet_v2_set_recording_permission','meet_v2_set_recording_state','meet_v2_transfer_host_and_leave','meet_v2_end_room'])assert(service.includes(rpc),`Missing meeting RPC ${rpc}`);
...
for(const channel of ['meeting:create','meeting:personal-room','meeting:update-personal-room','meeting:start-personal-room','meeting:schedule','meeting:list-schedules','meeting:start-schedule','meeting:cancel-schedule','meeting:request-join','meeting:join-status','meeting:mark-joined','meeting:leave','meeting:host-queue','meeting:decide','meeting:snapshot','meeting:set-cohost','meeting:remove-participant','meeting:rename-participant','meeting:set-security','meeting:set-chat-policy','meeting:set-caption-state','meeting:publish-caption','meeting:get-transcript','meeting:set-recording-permission','meeting:set-recording-state','meeting:transfer-host-and-leave','meeting:end'])assert(main.includes(channel),`Missing IPC owner ${channel}`);
for(const method of ['create:','personalRoom:','updatePersonalRoom:','startPersonalRoom:','schedule:','listSchedules:','startSchedule:','cancelSchedule:','requestJoin:','joinStatus:','markJoined:','leave:','hostQueue:','decide:','snapshot:','setCohost:','removeParticipant:','renameParticipant:','setSecurity:','setChatPolicy:','setCaptionState:','publishCaption:','transcript:','setRecordingPermission:','setRecordingState:','transferHostAndLeave:','end:'])assert(preload.includes(method),`Missing renderer bridge method ${method}`);
...
assert(migration.includes("caption_mode text not null default 'off'")&&migration.includes("caption_mode in ('off','manual')"),'Live caption mode must be server-backed and constrained to real supported modes.');
assert(migration.includes('captioner_participant_id uuid references public.meet_v2_participants(id)')&&migration.includes('transcript_enabled boolean not null default false'),'Captioner assignment and transcript retention must be server-backed and transcript must default off.');
assert(migration.includes('create table if not exists public.meet_v2_transcript_lines'),'Retained transcript lines must have a dedicated persistent store separate from live caption display history.');
assert(migration.includes('meet_v2_set_caption_state')&&migration.includes("if coalesce(v_room.active_host_id,v_room.host_id)<>v_user then raise exception 'host_authority_required'"),'Only the active host may configure manual captions or transcript retention.');
assert(migration.includes('meet_v2_publish_caption')&&migration.includes("v_room.caption_mode<>'manual' or v_room.captioner_participant_id<>v_participant.id")&&migration.includes('v_participant.member_id is distinct from v_user'),'Only the assigned authenticated manual captioner may publish caption text.');
assert(migration.includes('if v_room.transcript_enabled then')&&migration.includes('insert into public.meet_v2_transcript_lines'),'Caption text must only be retained when transcript retention is explicitly enabled.');
assert(migration.includes('meet_v2_get_transcript')&&migration.includes("transcript_access_denied"),'Retained transcript retrieval must have explicit authority checks.');
assert(migration.includes("'caption'")&&migration.includes("'caption-request'"),'Authenticated signaling must explicitly allow live caption delivery and participant caption requests.');
assert(migration.includes('recording_allowed boolean not null default false')&&migration.includes('is_recording boolean not null default false')&&migration.includes('recording_paused boolean not null default false'),'Recording permission, active state, and paused state must be participant-scoped server state.');
assert(migration.includes('meet_v2_set_recording_permission')&&migration.includes("coalesce(v_room.active_host_id,v_room.host_id)<>v_actor"),'Only the active host may grant or revoke ordinary participant recording permission.');
assert(migration.includes('meet_v2_set_recording_state')&&migration.includes("v_participant.role not in ('host','cohost') and not coalesce(v_participant.recording_allowed,false)"),'Only host/co-host or explicitly allowed participants may mark themselves as recording.');
assert(migration.includes("p_signal_type='recording-state'")&&migration.includes("raise exception 'recording_authority_required'"),'Recording-state signaling must reject unauthorized senders in backend authority.');
assert(migration.includes("'recordingAllowed',p.recording_allowed,'isRecording',p.is_recording,'recordingPaused',p.recording_paused"),'Room snapshots must expose recording permission, active state, and paused state for late joiners.');
assert(migration.includes("recording_paused=case when coalesce(p_active,false) then coalesce(p_paused,false) else false end"),'Recording pause state must clear whenever recording stops.');
assert(migration.includes("is_recording=case when coalesce(p_enabled,false) then is_recording else false end")&&migration.includes("recording_paused=case when coalesce(p_enabled,false) then recording_paused else false end"),'Revoking recording permission must atomically clear backend active/paused recording state.');
...
console.log('DOMINIONSTAR_MEET_V2_LIFECYCLE_OK personal-id generated-id passcode-3-7 reusable-waiting recurring-identity schedule prejoin waiting admit cohost atomic-host-handoff raise-hand-authority captions transcript-retention recording-authority recording-state leave end');
