...
try{
  ...
  await evaluateDiagnosed(`(()=>{
    ...
    window.DominionMeetingParity.install();window.DominionMeetingFeatures.toggleChat(false);window.DominionApprovedReferenceParity.sync();
    return true;
  ...
  await evaluate(`window.DominionMeetingParity.install();window.DominionMeetingParity.decorateControls();window.DominionApprovedReferenceParity.sync();true`);
  ...
  assert.deepEqual(toolbarGeometryAfter,toolbarGeometryBefore,'Primary toolbar geometry changed while idle; controls must not dance left/right during periodic reconciliation.');
  ...
  const participantPanelGeometry=await evaluate(`(()=>{window.DominionZoomAdaptiveParity.syncParticipants();const panel=document.querySelector('.room-side'),body=document.querySelector('.meeting-body'),side=panel.getBoundingClientRect(),br=body.getBoundingClientRect();return {width:Math.round(side.width),height:Math.round(side.height),centerDelta:Math.round(Math.abs((side.left+side.width/2)-(br.left+br.width/2))),position:getComputedStyle(panel).position,mode:panel.dataset.zoomPanelMode,adaptive:panel.dataset.dsAdaptiveMode};})()`);
  assert.equal(participantPanelGeometry.position,'absolute','Participant management panel must float over the meeting rather than shrink the stage.');
  assert.equal(participantPanelGeometry.mode,'popout','Small-roster Participants must open as the approved floating panel.');
  assert.equal(participantPanelGeometry.adaptive,'floating','Small-roster Participants must use adaptive floating geometry.');
  assert.ok(participantPanelGeometry.width<=345,'Small-roster Participants panel is wider than the approved compact Zoom reference.');
  assert.ok(participantPanelGeometry.centerDelta<=60,'Small-roster Participants panel must initially float near the meeting center.');
  ...
  console.log('DOMINIONSTAR_PACKAGED_INTERACTIONS_OK home-dialogs settings personal-room schedule recurrence approved-toolbar-stable participants-floating clean-chat reactions more adaptive-full-stage-dock');
}catch(error){
  failure=error;
